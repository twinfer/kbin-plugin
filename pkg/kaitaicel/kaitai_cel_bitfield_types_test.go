...
func TestKaitaiBitField_Equal(t *testing.T) {
	...
	assert.True(t, bf_5_val21a.Equal(bf_5_val21b).(types.Bool))
	assert.False(t, bf_5_val21a.Equal(bf_5_val10).(types.Bool))
	assert.False(t, bf_5_val21a.Equal(bf_3_val5).(types.Bool), "Different bit counts")

	assert.True(t, bf_5_val21a.Equal(types.Int(21)).(types.Bool))
	assert.False(t, bf_5_val21a.Equal(types.Int(10)).(types.Bool))
	assert.True(t, bf_5_val21a.Equal(types.Uint(21)).(types.Bool))
	
	bf_1_true, _ := NewKaitaiBitField(1,1)
	bf_1_false, _ := NewKaitaiBitField(0,1)
	assert.True(t, bf_1_true.Equal(types.True).(types.Bool))
	assert.False(t, bf_1_true.Equal(types.False).(types.Bool))
	assert.True(t, bf_1_false.Equal(types.False).(types.Bool))
	assert.False(t, bf_1_false.Equal(types.True).(types.Bool))


	assert.False(t, bf_5_val21a.Equal(types.String("21")).(types.Bool))
}
...
func TestKaitaiBitField_ConvertToNative(t *testing.T) {
	...
	require.NoError(t, err); assert.Equal(t, true, bVal)
	bVal, err = bf_false.ConvertToNative(reflect.TypeOf(true))
	require.NoError(t, err); assert.Equal(t, false, bVal)

	iVal, err := bf_val42.ConvertToNative(reflect.TypeOf(int64(0)))
	require.NoError(t, err); assert.Equal(t, int64(42), iVal)
	
	uVal, err := bf_val42.ConvertToNative(reflect.TypeOf(uint64(0)))
	require.NoError(t, err); assert.Equal(t, uint64(42), uVal)
...
func TestBitReader_ReadBits_BigEndian(t *testing.T) {
	...
	require.NoError(t, err); assert.Equal(t, uint64(0b101), bf.AsUint()); assert.Equal(t, 3, bf.BitCount())
	assert.Equal(t, 0, br.bytePos); assert.Equal(t, 3, br.bitPos)

	// Read 4 bits from 10110100 -> 1010 (10)
	bf, err = br.ReadBits(4)
	require.NoError(t, err); assert.Equal(t, uint64(0b1010), bf.AsUint()); assert.Equal(t, 4, bf.BitCount())
	assert.Equal(t, 0, br.bytePos); assert.Equal(t, 7, br.bitPos)

	// Read 2 bits (1 from 0xB4: 0, 1 from 0x5A: 0) -> 00 (0)
	// 0xB4: 1011010(0) - remaining bit
	// 0x5A: (0)1011010 - first bit
	bf, err = br.ReadBits(2)
	require.NoError(t, err); assert.Equal(t, uint64(0b00), bf.AsUint()); assert.Equal(t, 2, bf.BitCount())
	assert.Equal(t, 1, br.bytePos); assert.Equal(t, 1, br.bitPos) // bytePos advanced, bitPos is 1 (after reading 1st bit of 0x5A)
	
	// Read 7 bits from 01011010 (remaining after 1st bit: 1011010) -> 1011010 (90)
	bf, err = br.ReadBits(7)
	require.NoError(t, err); assert.Equal(t, uint64(0b1011010), bf.AsUint()); assert.Equal(t, 7, bf.BitCount())
	assert.Equal(t, 2, br.bytePos); assert.Equal(t, 0, br.bitPos) // EOF (bytePos is 2, bitPos is 0, indicating end of data)
...
func TestBitReader_ReadBits_LittleEndian(t *testing.T) {
	...
	bf, err := br.ReadBits(3)
	require.NoError(t, err); assert.Equal(t, uint64(0b100), bf.AsUint()); assert.Equal(t, 3, bf.BitCount())
	assert.Equal(t, 0, br.bytePos); assert.Equal(t, 3, br.bitPos)

	// Read 4 bits from 10110100 -> 1011 (11) (next 4 bits: 1,0,1,1)
	bf, err = br.ReadBits(4)
	require.NoError(t, err); assert.Equal(t, uint64(0b1011), bf.AsUint()); assert.Equal(t, 4, bf.BitCount())
	assert.Equal(t, 0, br.bytePos); assert.Equal(t, 7, br.bitPos)
	...
	bf, err = br.ReadBits(2)
	require.NoError(t, err); assert.Equal(t, uint64(0b01), bf.AsUint()); assert.Equal(t, 2, bf.BitCount())
	assert.Equal(t, 1, br.bytePos); assert.Equal(t, 1, br.bitPos)

	// Read 7 bits from 01011010 (remaining after 1st bit: 0101101) -> 0101101 (45)
	// (Bits read: 1,0,1,1,0,1,0)
	bf, err = br.ReadBits(7)
	require.NoError(t, err); assert.Equal(t, uint64(0b0101101), bf.AsUint()); assert.Equal(t, 7, bf.BitCount())
	assert.Equal(t, 2, br.bytePos); assert.Equal(t, 0, br.bitPos) // EOF
...
func TestBitReader_ReadBits_ErrorCases(t *testing.T) {
	...
	_, err := br.ReadBits(0)
	assert.Error(t, err); assert.Contains(t, err.Error(), "bit count must be between 1 and 64")
	...
	assert.Error(t, err); assert.Contains(t, err.Error(), "EOF")
}
