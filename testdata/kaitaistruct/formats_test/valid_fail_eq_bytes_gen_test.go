...
import (
	...
	// The alias helps avoid name collisions if valid_fail_eq_bytes is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailEqBytes(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailEqBytes(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailEqBytes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_eq_bytes.ksy")
	ksySchema := loadKsySchemaForValidFailEqBytes(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_eq_bytes, skipping detailed parse tests.")

}

func TestSerialize_ValidFailEqBytes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_eq_bytes.ksy")
	ksySchema := loadKsySchemaForValidFailEqBytes(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_eq_bytes, skipping detailed serialize tests.")
