...
import (
	...
	valid_fail_eq_bytes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_eq_bytes"
)

func TestParse_ValidFailEqBytes(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_eq_bytes.ksy")
...
func TestSerialize_ValidFailEqBytes(t *testing.T) {
	...
	original := valid_fail_eq_bytes_kaitai.NewValidFailEqBytes()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailEqBytes(t, original)
	...
	reparsed := valid_fail_eq_bytes_kaitai.NewValidFailEqBytes()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_eq_bytes_kaitai.ValidFailEqBytes{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailEqBytes converts KSC struct to map for serializer
func structToMapValidFailEqBytes(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailEqBytes(t, data)
}

// structToMapReflectiveValidFailEqBytes uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailEqBytes(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailEqBytes(t, fieldVal.Interface())
