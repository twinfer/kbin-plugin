...
import (
	...
	// The alias helps avoid name collisions if combine_bytes is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForCombineBytes(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForCombineBytes(t *testing.T, data any) map[string]any {
...
func TestParse_CombineBytes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "combine_bytes.ksy")
	ksySchema := loadKsySchemaForCombineBytes(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for combine_bytes, skipping detailed parse tests.")
...
func TestSerialize_CombineBytes(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for combine_bytes, skipping detailed serialize tests.")
