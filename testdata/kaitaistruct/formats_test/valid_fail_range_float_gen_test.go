...
import (
	...
	valid_fail_range_float_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_range_float"
)

func TestParse_ValidFailRangeFloat(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_range_float.ksy")
...
func TestSerialize_ValidFailRangeFloat(t *testing.T) {
	...
	original := valid_fail_range_float_kaitai.NewValidFailRangeFloat()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailRangeFloat(t, original)
	...
	reparsed := valid_fail_range_float_kaitai.NewValidFailRangeFloat()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_range_float_kaitai.ValidFailRangeFloat{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailRangeFloat converts KSC struct to map for serializer
func structToMapValidFailRangeFloat(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailRangeFloat(t, data)
}

// structToMapReflectiveValidFailRangeFloat uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailRangeFloat(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailRangeFloat(t, fieldVal.Interface())
