...
import (
	...
	// The alias helps avoid name collisions if buffered_struct is a common word.
...
func loadKsySchemaForBufferedStruct(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForBufferedStruct(t *testing.T, data any) map[string]any {
...
func TestParse_BufferedStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "buffered_struct.ksy")
	ksySchema := loadKsySchemaForBufferedStruct(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("buffered_struct_buffered_struct_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "buffered_struct.bin")
		...
		kscParsed := buffered_struct_kaitai.NewBufferedStruct() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for buffered_struct (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_BufferedStruct(t *testing.T) {
	...
	t.Run("buffered_struct_buffered_struct_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "buffered_struct.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := buffered_struct_kaitai.NewBufferedStruct()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForBufferedStruct(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := buffered_struct_kaitai.NewBufferedStruct()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for buffered_struct (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
