...
func TestParse_TypeIntUnaryOp(t *testing.T) {
	...
	// Assert ValueS2
	if values2_val, ok := customMap["ValueS2"]; ok {
		assert.EqualValues(t, 16720, values2_val)
	} else {
		t.Fatalf("Field 'ValueS2' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert ValueS8
	if values8_val, ok := customMap["ValueS8"]; ok {
		assert.EqualValues(t, int64(4706543082108963651), values8_val)
	} else {
		t.Fatalf("Field 'ValueS8' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert UnaryS2
	if unarys2_val, ok := customMap["UnaryS2"]; ok {
		assert.EqualValues(t, -16720, unarys2_val)
	} else {
		t.Fatalf("Field 'UnaryS2' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert UnaryS8
	if unarys8_val, ok := customMap["UnaryS8"]; ok {
		assert.EqualValues(t, int64(-4706543082108963651), unarys8_val)
	} else {
		t.Fatalf("Field 'UnaryS8' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
