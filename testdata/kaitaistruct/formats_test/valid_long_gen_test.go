...
import (
	...
	valid_long_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_long"
)

func TestParse_ValidLong(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_long.ksy")
...
func TestSerialize_ValidLong(t *testing.T) {
	...
	original := valid_long_kaitai.NewValidLong()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidLong(t, original)
	...
	reparsed := valid_long_kaitai.NewValidLong()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_long_kaitai.ValidLong{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidLong converts KSC struct to map for serializer
func structToMapValidLong(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidLong(t, data)
}

// structToMapReflectiveValidLong uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidLong(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidLong(t, fieldVal.Interface())
