...
import (
	...
	eof_exception_bytes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/eof_exception_bytes"
)

func TestParse_EofExceptionBytes(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "eof_exception_bytes.ksy")
...
func TestSerialize_EofExceptionBytes(t *testing.T) {
	...
	original := eof_exception_bytes_kaitai.NewEofExceptionBytes()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEofExceptionBytes(t, original)
	...
	reparsed := eof_exception_bytes_kaitai.NewEofExceptionBytes()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(eof_exception_bytes_kaitai.EofExceptionBytes{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEofExceptionBytes converts KSC struct to map for serializer
func structToMapEofExceptionBytes(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEofExceptionBytes(t, data)
}

// structToMapReflectiveEofExceptionBytes uses reflection to convert KSC structs including method calls
func structToMapReflectiveEofExceptionBytes(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEofExceptionBytes(t, fieldVal.Interface())
