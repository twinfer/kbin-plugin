...
func TestParse_StrPadTerm(t *testing.T) {
	...
	if strpad_val, ok := customMap["StrPad"]; ok {
		assert.EqualValues(t, "str1", strpad_val)
	} else {
	...
	if strterm_val, ok := customMap["StrTerm"]; ok {
		assert.EqualValues(t, "str2foo", strterm_val)
	} else {
	...
	if strtermandpad_val, ok := customMap["StrTermAndPad"]; ok {
		assert.EqualValues(t, "str+++3bar+++", strtermandpad_val)
	} else {
	...
	if strterminclude_val, ok := customMap["StrTermInclude"]; ok {
		assert.EqualValues(t, "str4baz@", strterminclude_val)
	} else {
