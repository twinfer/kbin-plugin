...
import (
	...
	// The alias helps avoid name collisions if str_pad_term is a common word.
	str_pad_term_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/str_pad_term"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForStrPadTerm(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForStrPadTerm(t *testing.T, data any) map[string]any {
...
func TestParse_StrPadTerm(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_pad_term.ksy")
	ksySchema := loadKsySchemaForStrPadTerm(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("str_pad_term_str_pad_term_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_pad_term.bin")
		...
		kscParsed := str_pad_term_kaitai.NewStrPadTerm() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for str_pad_term (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_StrPadTerm(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_pad_term.ksy")
	ksySchema := loadKsySchemaForStrPadTerm(t, ksyFilePath)
	...
	t.Run("str_pad_term_str_pad_term_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_pad_term.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := str_pad_term_kaitai.NewStrPadTerm()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForStrPadTerm(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := str_pad_term_kaitai.NewStrPadTerm()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for str_pad_term (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
