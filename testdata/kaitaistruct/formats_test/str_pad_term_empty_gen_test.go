...
import (
	...
	// The alias helps avoid name collisions if str_pad_term_empty is a common word.
...
func loadKsySchemaForStrPadTermEmpty(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForStrPadTermEmpty(t *testing.T, data any) map[string]any {
...
func TestParse_StrPadTermEmpty(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_pad_term_empty.ksy")
	ksySchema := loadKsySchemaForStrPadTermEmpty(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("str_pad_term_empty_str_pad_term_empty_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_pad_term_empty.bin")
		...
		kscParsed := str_pad_term_empty_kaitai.NewStrPadTermEmpty() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for str_pad_term_empty (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_StrPadTermEmpty(t *testing.T) {
	...
	t.Run("str_pad_term_empty_str_pad_term_empty_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_pad_term_empty.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := str_pad_term_empty_kaitai.NewStrPadTermEmpty()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForStrPadTermEmpty(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := str_pad_term_empty_kaitai.NewStrPadTermEmpty()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for str_pad_term_empty (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
