...
import (
	...
	// The alias helps avoid name collisions if switch_manual_int_else is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForSwitchManualIntElse(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForSwitchManualIntElse(t *testing.T, data any) map[string]any {
...
func TestParse_SwitchManualIntElse(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_manual_int_else.ksy")
	ksySchema := loadKsySchemaForSwitchManualIntElse(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for switch_manual_int_else, skipping detailed parse tests.")

}

func TestSerialize_SwitchManualIntElse(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_manual_int_else.ksy")
	ksySchema := loadKsySchemaForSwitchManualIntElse(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for switch_manual_int_else, skipping detailed serialize tests.")
