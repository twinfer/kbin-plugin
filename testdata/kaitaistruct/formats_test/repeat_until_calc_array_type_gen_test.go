...
import (
	...
	// The alias helps avoid name collisions if repeat_until_calc_array_type is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForRepeatUntilCalcArrayType(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForRepeatUntilCalcArrayType(t *testing.T, data any) map[string]any {
...
func TestParse_RepeatUntilCalcArrayType(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_until_calc_array_type.ksy")
	ksySchema := loadKsySchemaForRepeatUntilCalcArrayType(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for repeat_until_calc_array_type, skipping detailed parse tests.")

}

func TestSerialize_RepeatUntilCalcArrayType(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_until_calc_array_type.ksy")
	ksySchema := loadKsySchemaForRepeatUntilCalcArrayType(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for repeat_until_calc_array_type, skipping detailed serialize tests.")
