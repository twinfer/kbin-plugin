...
import (
	...
	bits_enum_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/bits_enum"
)

func TestParse_BitsEnum(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "bits_enum.ksy")
	...
	if one_val, ok := customMap["One"]; ok {
		assert.EqualValues(t, BitsEnum_Animal__Platypus, one_val)
	} else {
	...
	if two_val, ok := customMap["Two"]; ok {
		assert.EqualValues(t, BitsEnum_Animal__Horse, two_val)
	} else {
	...
	if three_val, ok := customMap["Three"]; ok {
		assert.EqualValues(t, BitsEnum_Animal__Cat, three_val)
	} else {
...
func TestSerialize_BitsEnum(t *testing.T) {
	...
	original := bits_enum_kaitai.NewBitsEnum()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapBitsEnum(t, original)
	...
	reparsed := bits_enum_kaitai.NewBitsEnum()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(bits_enum_kaitai.BitsEnum{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapBitsEnum converts KSC struct to map for serializer
func structToMapBitsEnum(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveBitsEnum(t, data)
}

// structToMapReflectiveBitsEnum uses reflection to convert KSC structs including method calls
func structToMapReflectiveBitsEnum(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveBitsEnum(t, fieldVal.Interface())
