...
func TestParse_JsSignedRightShift(t *testing.T) {
	...
	if shouldbe40000000_val, ok := customMap["ShouldBe40000000"]; ok {
		assert.EqualValues(t, 1073741824, shouldbe40000000_val)
	} else {
		t.Fatalf("Field 'ShouldBe40000000' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert ShouldBeA00000
	if shouldbea00000_val, ok := customMap["ShouldBeA00000"]; ok {
		assert.EqualValues(t, 10485760, shouldbea00000_val)
	} else {
		t.Fatalf("Field 'ShouldBeA00000' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
