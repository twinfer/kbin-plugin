...
func TestParse_Expr1(t *testing.T) {
	...
	// Assert LenOf1
	if lenof1_val, ok := customMap["LenOf1"]; ok {
		assert.EqualValues(t, 10, lenof1_val)
	} else {
		t.Fatalf("Field 'LenOf1' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert LenOf1Mod
	if lenof1mod_val, ok := customMap["LenOf1Mod"]; ok {
		assert.EqualValues(t, 8, lenof1mod_val)
	} else {
		t.Fatalf("Field 'LenOf1Mod' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
