...
import (
	...
	// The alias helps avoid name collisions if valid_short is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidShort(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidShort(t *testing.T, data any) map[string]any {
...
func TestParse_ValidShort(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_short.ksy")
	ksySchema := loadKsySchemaForValidShort(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_short, skipping detailed parse tests.")
...
func TestSerialize_ValidShort(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for valid_short, skipping detailed serialize tests.")
