...
import (
	...
	// The alias helps avoid name collisions if str_encodings_utf16 is a common word.
	str_encodings_utf16_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/str_encodings_utf16"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForStrEncodingsUtf16(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForStrEncodingsUtf16(t *testing.T, data any) map[string]any {
...
func TestParse_StrEncodingsUtf16(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_encodings_utf16.ksy")
	ksySchema := loadKsySchemaForStrEncodingsUtf16(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("str_encodings_utf16_str_encodings_utf16_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_encodings_utf16.bin")
		...
		kscParsed := str_encodings_utf16_kaitai.NewStrEncodingsUtf16() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for str_encodings_utf16 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_StrEncodingsUtf16(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_encodings_utf16.ksy")
	ksySchema := loadKsySchemaForStrEncodingsUtf16(t, ksyFilePath)
	...
	t.Run("str_encodings_utf16_str_encodings_utf16_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_encodings_utf16.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := str_encodings_utf16_kaitai.NewStrEncodingsUtf16()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForStrEncodingsUtf16(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := str_encodings_utf16_kaitai.NewStrEncodingsUtf16()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for str_encodings_utf16 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
