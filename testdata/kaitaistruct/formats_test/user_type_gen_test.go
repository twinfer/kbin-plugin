...
import (
	...
	// The alias helps avoid name collisions if user_type is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForUserType(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForUserType(t *testing.T, data any) map[string]any {
...
func TestParse_UserType(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "user_type.ksy")
	ksySchema := loadKsySchemaForUserType(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for user_type, skipping detailed parse tests.")
...
func TestSerialize_UserType(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for user_type, skipping detailed serialize tests.")
