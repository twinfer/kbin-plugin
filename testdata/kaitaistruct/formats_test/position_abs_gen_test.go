...
func TestParse_PositionAbs(t *testing.T) {
	...
	// Assert IndexOffset
	if indexoffset_val, ok := customMap["IndexOffset"]; ok {
		assert.EqualValues(t, 32, indexoffset_val)
	} else {
		t.Fatalf("Field 'IndexOffset' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
