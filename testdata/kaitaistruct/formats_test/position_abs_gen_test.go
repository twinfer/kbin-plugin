...
import (
	...
	// The alias helps avoid name collisions if position_abs is a common word.
	position_abs_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/position_abs"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForPositionAbs(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForPositionAbs(t *testing.T, data any) map[string]any {
...
func TestParse_PositionAbs(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "position_abs.ksy")
	ksySchema := loadKsySchemaForPositionAbs(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("position_abs_position_abs_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "position_abs.bin")
		...
		kscParsed := position_abs_kaitai.NewPositionAbs() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for position_abs (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_PositionAbs(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "position_abs.ksy")
	ksySchema := loadKsySchemaForPositionAbs(t, ksyFilePath)
	...
	t.Run("position_abs_position_abs_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "position_abs.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := position_abs_kaitai.NewPositionAbs()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForPositionAbs(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := position_abs_kaitai.NewPositionAbs()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for position_abs (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
