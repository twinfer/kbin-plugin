...
import (
	...
	// The alias helps avoid name collisions if term_strz is a common word.
	term_strz_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/term_strz"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForTermStrz(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForTermStrz(t *testing.T, data any) map[string]any {
...
func TestParse_TermStrz(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "term_strz.ksy")
	ksySchema := loadKsySchemaForTermStrz(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("term_strz_term_strz_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "term_strz.bin")
		...
		kscParsed := term_strz_kaitai.NewTermStrz() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for term_strz (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_TermStrz(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "term_strz.ksy")
	ksySchema := loadKsySchemaForTermStrz(t, ksyFilePath)
	...
	t.Run("term_strz_term_strz_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "term_strz.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := term_strz_kaitai.NewTermStrz()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForTermStrz(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := term_strz_kaitai.NewTermStrz()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for term_strz (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
