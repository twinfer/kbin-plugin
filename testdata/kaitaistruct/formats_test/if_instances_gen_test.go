...
import (
	...
	if_instances_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/if_instances"
)

func TestParse_IfInstances(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "if_instances.ksy")
...
func TestSerialize_IfInstances(t *testing.T) {
	...
	original := if_instances_kaitai.NewIfInstances()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapIfInstances(t, original)
	...
	reparsed := if_instances_kaitai.NewIfInstances()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(if_instances_kaitai.IfInstances{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapIfInstances converts KSC struct to map for serializer
func structToMapIfInstances(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveIfInstances(t, data)
}

// structToMapReflectiveIfInstances uses reflection to convert KSC structs including method calls
func structToMapReflectiveIfInstances(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveIfInstances(t, fieldVal.Interface())
