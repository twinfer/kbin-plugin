...
import (
	...
	docstrings_docref_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/docstrings_docref"
)

func TestParse_DocstringsDocref(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "docstrings_docref.ksy")
...
func TestSerialize_DocstringsDocref(t *testing.T) {
	...
	original := docstrings_docref_kaitai.NewDocstringsDocref()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapDocstringsDocref(t, original)
	...
	reparsed := docstrings_docref_kaitai.NewDocstringsDocref()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(docstrings_docref_kaitai.DocstringsDocref{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapDocstringsDocref converts KSC struct to map for serializer
func structToMapDocstringsDocref(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveDocstringsDocref(t, data)
}

// structToMapReflectiveDocstringsDocref uses reflection to convert KSC structs including method calls
func structToMapReflectiveDocstringsDocref(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveDocstringsDocref(t, fieldVal.Interface())
