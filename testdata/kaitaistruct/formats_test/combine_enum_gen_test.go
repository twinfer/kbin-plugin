...
import (
	...
	combine_enum_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/combine_enum"
)

func TestParse_CombineEnum(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "combine_enum.ksy")
	...
	// Assert EnumU4
	if enumu4_val, ok := customMap["EnumU4"]; ok {
		assert.EqualValues(t, CombineEnum_Animal__Pig, enumu4_val)
	} else {
		t.Fatalf("Field 'EnumU4' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert EnumU2
	if enumu2_val, ok := customMap["EnumU2"]; ok {
		assert.EqualValues(t, CombineEnum_Animal__Horse, enumu2_val)
	} else {
		t.Fatalf("Field 'EnumU2' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert EnumU4U2
	if enumu4u2_val, ok := customMap["EnumU4U2"]; ok {
		assert.EqualValues(t, CombineEnum_Animal__Horse, enumu4u2_val)
	} else {
		t.Fatalf("Field 'EnumU4U2' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
...
func TestSerialize_CombineEnum(t *testing.T) {
	...
	original := combine_enum_kaitai.NewCombineEnum()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapCombineEnum(t, original)
	...
	reparsed := combine_enum_kaitai.NewCombineEnum()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(combine_enum_kaitai.CombineEnum{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapCombineEnum converts KSC struct to map for serializer
func structToMapCombineEnum(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveCombineEnum(t, data)
}

// structToMapReflectiveCombineEnum uses reflection to convert KSC structs including method calls
func structToMapReflectiveCombineEnum(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveCombineEnum(t, fieldVal.Interface())
