...
import (
	...
	docstrings_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/docstrings"
)

func TestParse_Docstrings(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "docstrings.ksy")
...
func TestSerialize_Docstrings(t *testing.T) {
	...
	original := docstrings_kaitai.NewDocstrings()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapDocstrings(t, original)
	...
	reparsed := docstrings_kaitai.NewDocstrings()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(docstrings_kaitai.Docstrings{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapDocstrings converts KSC struct to map for serializer
func structToMapDocstrings(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveDocstrings(t, data)
}

// structToMapReflectiveDocstrings uses reflection to convert KSC structs including method calls
func structToMapReflectiveDocstrings(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveDocstrings(t, fieldVal.Interface())
