...
import (
	...
	// The alias helps avoid name collisions if nested_same_name2 is a common word.
...
func loadKsySchemaForNestedSameName2(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForNestedSameName2(t *testing.T, data any) map[string]any {
...
func TestParse_NestedSameName2(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nested_same_name2.ksy")
	ksySchema := loadKsySchemaForNestedSameName2(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("nested_same_name2_nested_same_name2_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "nested_same_name2.bin")
		...
		kscParsed := nested_same_name2_kaitai.NewNestedSameName2() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for nested_same_name2 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_NestedSameName2(t *testing.T) {
	...
	t.Run("nested_same_name2_nested_same_name2_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "nested_same_name2.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := nested_same_name2_kaitai.NewNestedSameName2()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForNestedSameName2(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := nested_same_name2_kaitai.NewNestedSameName2()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for nested_same_name2 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
