...
import (
	...
	// The alias helps avoid name collisions if enum_long_range_s is a common word.
...
func loadKsySchemaForEnumLongRangeS(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumLongRangeS(t *testing.T, data any) map[string]any {
...
func TestParse_EnumLongRangeS(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_long_range_s.ksy")
	ksySchema := loadKsySchemaForEnumLongRangeS(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("enum_long_range_s_enum_long_range_s_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_long_range_s.bin")
		...
		kscParsed := enum_long_range_s_kaitai.NewEnumLongRangeS() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for enum_long_range_s (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_EnumLongRangeS(t *testing.T) {
	...
	t.Run("enum_long_range_s_enum_long_range_s_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_long_range_s.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := enum_long_range_s_kaitai.NewEnumLongRangeS()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForEnumLongRangeS(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := enum_long_range_s_kaitai.NewEnumLongRangeS()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for enum_long_range_s (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
