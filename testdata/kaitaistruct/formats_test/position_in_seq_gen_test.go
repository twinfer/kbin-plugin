...
import (
	...
	// The alias helps avoid name collisions if position_in_seq is a common word.
	position_in_seq_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/position_in_seq"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForPositionInSeq(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForPositionInSeq(t *testing.T, data any) map[string]any {
...
func TestParse_PositionInSeq(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "position_in_seq.ksy")
	ksySchema := loadKsySchemaForPositionInSeq(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("position_in_seq_position_in_seq_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "position_in_seq.bin")
		...
		kscParsed := position_in_seq_kaitai.NewPositionInSeq() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for position_in_seq (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_PositionInSeq(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "position_in_seq.ksy")
	ksySchema := loadKsySchemaForPositionInSeq(t, ksyFilePath)
	...
	t.Run("position_in_seq_position_in_seq_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "position_in_seq.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := position_in_seq_kaitai.NewPositionInSeq()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForPositionInSeq(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := position_in_seq_kaitai.NewPositionInSeq()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for position_in_seq (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
