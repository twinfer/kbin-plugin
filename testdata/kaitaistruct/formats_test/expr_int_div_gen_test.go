...
import (
	...
	// The alias helps avoid name collisions if expr_int_div is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForExprIntDiv(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprIntDiv(t *testing.T, data any) map[string]any {
...
func TestParse_ExprIntDiv(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_int_div.ksy")
	ksySchema := loadKsySchemaForExprIntDiv(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for expr_int_div, skipping detailed parse tests.")
...
func TestSerialize_ExprIntDiv(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for expr_int_div, skipping detailed serialize tests.")
