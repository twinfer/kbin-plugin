...
import (
	...
	// The alias helps avoid name collisions if valid_fail_contents is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailContents(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailContents(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailContents(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_contents.ksy")
	ksySchema := loadKsySchemaForValidFailContents(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_contents, skipping detailed parse tests.")

}

func TestSerialize_ValidFailContents(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_contents.ksy")
	ksySchema := loadKsySchemaForValidFailContents(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_contents, skipping detailed serialize tests.")
