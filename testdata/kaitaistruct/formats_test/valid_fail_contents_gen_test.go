...
import (
	...
	valid_fail_contents_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_contents"
)

func TestParse_ValidFailContents(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_contents.ksy")
...
func TestSerialize_ValidFailContents(t *testing.T) {
	...
	original := valid_fail_contents_kaitai.NewValidFailContents()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailContents(t, original)
	...
	reparsed := valid_fail_contents_kaitai.NewValidFailContents()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_contents_kaitai.ValidFailContents{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailContents converts KSC struct to map for serializer
func structToMapValidFailContents(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailContents(t, data)
}

// structToMapReflectiveValidFailContents uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailContents(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailContents(t, fieldVal.Interface())
