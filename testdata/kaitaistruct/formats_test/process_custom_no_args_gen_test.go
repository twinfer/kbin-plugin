...
import (
	...
	// The alias helps avoid name collisions if process_custom_no_args is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForProcessCustomNoArgs(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForProcessCustomNoArgs(t *testing.T, data any) map[string]any {
...
func TestParse_ProcessCustomNoArgs(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_custom_no_args.ksy")
	ksySchema := loadKsySchemaForProcessCustomNoArgs(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for process_custom_no_args, skipping detailed parse tests.")
...
func TestSerialize_ProcessCustomNoArgs(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for process_custom_no_args, skipping detailed serialize tests.")
