...
import (
	...
	eof_exception_u4_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/eof_exception_u4"
)

func TestParse_EofExceptionU4(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "eof_exception_u4.ksy")
...
func TestSerialize_EofExceptionU4(t *testing.T) {
	...
	original := eof_exception_u4_kaitai.NewEofExceptionU4()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEofExceptionU4(t, original)
	...
	reparsed := eof_exception_u4_kaitai.NewEofExceptionU4()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(eof_exception_u4_kaitai.EofExceptionU4{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEofExceptionU4 converts KSC struct to map for serializer
func structToMapEofExceptionU4(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEofExceptionU4(t, data)
}

// structToMapReflectiveEofExceptionU4 uses reflection to convert KSC structs including method calls
func structToMapReflectiveEofExceptionU4(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEofExceptionU4(t, fieldVal.Interface())
