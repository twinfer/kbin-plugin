...
import (
	...
	// The alias helps avoid name collisions if valid_not_parsed_if is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidNotParsedIf(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidNotParsedIf(t *testing.T, data any) map[string]any {
...
func TestParse_ValidNotParsedIf(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_not_parsed_if.ksy")
	ksySchema := loadKsySchemaForValidNotParsedIf(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_not_parsed_if, skipping detailed parse tests.")

}

func TestSerialize_ValidNotParsedIf(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_not_parsed_if.ksy")
	ksySchema := loadKsySchemaForValidNotParsedIf(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_not_parsed_if, skipping detailed serialize tests.")
