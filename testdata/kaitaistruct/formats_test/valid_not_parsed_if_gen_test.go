...
import (
	...
	valid_not_parsed_if_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_not_parsed_if"
)

func TestParse_ValidNotParsedIf(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_not_parsed_if.ksy")
...
func TestSerialize_ValidNotParsedIf(t *testing.T) {
	...
	original := valid_not_parsed_if_kaitai.NewValidNotParsedIf()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidNotParsedIf(t, original)
	...
	reparsed := valid_not_parsed_if_kaitai.NewValidNotParsedIf()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_not_parsed_if_kaitai.ValidNotParsedIf{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidNotParsedIf converts KSC struct to map for serializer
func structToMapValidNotParsedIf(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidNotParsedIf(t, data)
}

// structToMapReflectiveValidNotParsedIf uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidNotParsedIf(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidNotParsedIf(t, fieldVal.Interface())
