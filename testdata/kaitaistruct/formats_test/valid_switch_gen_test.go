...
import (
	...
	valid_switch_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_switch"
)

func TestParse_ValidSwitch(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_switch.ksy")
...
func TestSerialize_ValidSwitch(t *testing.T) {
	...
	original := valid_switch_kaitai.NewValidSwitch()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidSwitch(t, original)
	...
	reparsed := valid_switch_kaitai.NewValidSwitch()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_switch_kaitai.ValidSwitch{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidSwitch converts KSC struct to map for serializer
func structToMapValidSwitch(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidSwitch(t, data)
}

// structToMapReflectiveValidSwitch uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidSwitch(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidSwitch(t, fieldVal.Interface())
