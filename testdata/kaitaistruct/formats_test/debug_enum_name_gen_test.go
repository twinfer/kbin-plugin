...
import (
	...
	// The alias helps avoid name collisions if debug_enum_name is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForDebugEnumName(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForDebugEnumName(t *testing.T, data any) map[string]any {
...
func TestParse_DebugEnumName(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "debug_enum_name.ksy")
	ksySchema := loadKsySchemaForDebugEnumName(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for debug_enum_name, skipping detailed parse tests.")
...
func TestSerialize_DebugEnumName(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for debug_enum_name, skipping detailed serialize tests.")
