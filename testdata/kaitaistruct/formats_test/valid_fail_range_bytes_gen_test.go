...
import (
	...
	valid_fail_range_bytes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_range_bytes"
)

func TestParse_ValidFailRangeBytes(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_range_bytes.ksy")
...
func TestSerialize_ValidFailRangeBytes(t *testing.T) {
	...
	original := valid_fail_range_bytes_kaitai.NewValidFailRangeBytes()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailRangeBytes(t, original)
	...
	reparsed := valid_fail_range_bytes_kaitai.NewValidFailRangeBytes()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_range_bytes_kaitai.ValidFailRangeBytes{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailRangeBytes converts KSC struct to map for serializer
func structToMapValidFailRangeBytes(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailRangeBytes(t, data)
}

// structToMapReflectiveValidFailRangeBytes uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailRangeBytes(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailRangeBytes(t, fieldVal.Interface())
