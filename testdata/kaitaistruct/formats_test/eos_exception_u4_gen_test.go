...
import (
	...
	eos_exception_u4_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/eos_exception_u4"
)

func TestParse_EosExceptionU4(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "eos_exception_u4.ksy")
...
func TestSerialize_EosExceptionU4(t *testing.T) {
	...
	original := eos_exception_u4_kaitai.NewEosExceptionU4()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEosExceptionU4(t, original)
	...
	reparsed := eos_exception_u4_kaitai.NewEosExceptionU4()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(eos_exception_u4_kaitai.EosExceptionU4{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEosExceptionU4 converts KSC struct to map for serializer
func structToMapEosExceptionU4(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEosExceptionU4(t, data)
}

// structToMapReflectiveEosExceptionU4 uses reflection to convert KSC structs including method calls
func structToMapReflectiveEosExceptionU4(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEosExceptionU4(t, fieldVal.Interface())
