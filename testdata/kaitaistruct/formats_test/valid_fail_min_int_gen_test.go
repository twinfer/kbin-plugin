...
import (
	...
	valid_fail_min_int_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_min_int"
)

func TestParse_ValidFailMinInt(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_min_int.ksy")
...
func TestSerialize_ValidFailMinInt(t *testing.T) {
	...
	original := valid_fail_min_int_kaitai.NewValidFailMinInt()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailMinInt(t, original)
	...
	reparsed := valid_fail_min_int_kaitai.NewValidFailMinInt()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_min_int_kaitai.ValidFailMinInt{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailMinInt converts KSC struct to map for serializer
func structToMapValidFailMinInt(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailMinInt(t, data)
}

// structToMapReflectiveValidFailMinInt uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailMinInt(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailMinInt(t, fieldVal.Interface())
