...
import (
	...
	// The alias helps avoid name collisions if ts_packet_header is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForTsPacketHeader(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForTsPacketHeader(t *testing.T, data any) map[string]any {
...
func TestParse_TsPacketHeader(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "ts_packet_header.ksy")
	ksySchema := loadKsySchemaForTsPacketHeader(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for ts_packet_header, skipping detailed parse tests.")

}

func TestSerialize_TsPacketHeader(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "ts_packet_header.ksy")
	ksySchema := loadKsySchemaForTsPacketHeader(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for ts_packet_header, skipping detailed serialize tests.")
