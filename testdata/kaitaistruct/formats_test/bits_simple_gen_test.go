...
import (
	...
	// The alias helps avoid name collisions if bits_simple is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForBitsSimple(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForBitsSimple(t *testing.T, data any) map[string]any {
...
func TestParse_BitsSimple(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bits_simple.ksy")
	ksySchema := loadKsySchemaForBitsSimple(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for bits_simple, skipping detailed parse tests.")
...
func TestSerialize_BitsSimple(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for bits_simple, skipping detailed serialize tests.")
