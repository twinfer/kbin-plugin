...
import (
	...
	// The alias helps avoid name collisions if nested_types is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForNestedTypes(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForNestedTypes(t *testing.T, data any) map[string]any {
...
func TestParse_NestedTypes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nested_types.ksy")
	ksySchema := loadKsySchemaForNestedTypes(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for nested_types, skipping detailed parse tests.")
...
func TestSerialize_NestedTypes(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for nested_types, skipping detailed serialize tests.")
