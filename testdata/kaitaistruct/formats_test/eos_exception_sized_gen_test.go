...
import (
	...
	eos_exception_sized_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/eos_exception_sized"
)

func TestParse_EosExceptionSized(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "eos_exception_sized.ksy")
...
func TestSerialize_EosExceptionSized(t *testing.T) {
	...
	original := eos_exception_sized_kaitai.NewEosExceptionSized()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEosExceptionSized(t, original)
	...
	reparsed := eos_exception_sized_kaitai.NewEosExceptionSized()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(eos_exception_sized_kaitai.EosExceptionSized{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEosExceptionSized converts KSC struct to map for serializer
func structToMapEosExceptionSized(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEosExceptionSized(t, data)
}

// structToMapReflectiveEosExceptionSized uses reflection to convert KSC structs including method calls
func structToMapReflectiveEosExceptionSized(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEosExceptionSized(t, fieldVal.Interface())
