...
import (
	...
	// The alias helps avoid name collisions if type_ternary is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForTypeTernary(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForTypeTernary(t *testing.T, data any) map[string]any {
...
func TestParse_TypeTernary(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "type_ternary.ksy")
	ksySchema := loadKsySchemaForTypeTernary(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for type_ternary, skipping detailed parse tests.")
...
func TestSerialize_TypeTernary(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for type_ternary, skipping detailed serialize tests.")
