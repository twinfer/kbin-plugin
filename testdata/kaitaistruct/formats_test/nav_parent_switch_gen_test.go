...
import (
	...
	// The alias helps avoid name collisions if nav_parent_switch is a common word.
	nav_parent_switch_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/nav_parent_switch"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForNavParentSwitch(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForNavParentSwitch(t *testing.T, data any) map[string]any {
...
func TestParse_NavParentSwitch(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nav_parent_switch.ksy")
	ksySchema := loadKsySchemaForNavParentSwitch(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("nav_parent_switch_nav_parent_switch_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "nav_parent_switch.bin")
		...
		kscParsed := nav_parent_switch_kaitai.NewNavParentSwitch() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for nav_parent_switch (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_NavParentSwitch(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nav_parent_switch.ksy")
	ksySchema := loadKsySchemaForNavParentSwitch(t, ksyFilePath)
	...
	t.Run("nav_parent_switch_nav_parent_switch_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "nav_parent_switch.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := nav_parent_switch_kaitai.NewNavParentSwitch()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForNavParentSwitch(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := nav_parent_switch_kaitai.NewNavParentSwitch()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for nav_parent_switch (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
