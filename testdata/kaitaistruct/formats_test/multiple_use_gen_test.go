...
import (
	...
	// The alias helps avoid name collisions if multiple_use is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForMultipleUse(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForMultipleUse(t *testing.T, data any) map[string]any {
...
func TestParse_MultipleUse(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "multiple_use.ksy")
	ksySchema := loadKsySchemaForMultipleUse(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for multiple_use, skipping detailed parse tests.")
...
func TestSerialize_MultipleUse(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for multiple_use, skipping detailed serialize tests.")
