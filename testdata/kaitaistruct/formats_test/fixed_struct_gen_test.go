...
import (
	...
	// The alias helps avoid name collisions if fixed_struct is a common word.
...
func loadKsySchemaForFixedStruct(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForFixedStruct(t *testing.T, data any) map[string]any {
...
func TestParse_FixedStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "fixed_struct.ksy")
	ksySchema := loadKsySchemaForFixedStruct(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("fixed_struct_fixed_struct_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "fixed_struct.bin")
		...
		kscParsed := fixed_struct_kaitai.NewFixedStruct() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for fixed_struct (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_FixedStruct(t *testing.T) {
	...
	t.Run("fixed_struct_fixed_struct_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "fixed_struct.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := fixed_struct_kaitai.NewFixedStruct()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForFixedStruct(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := fixed_struct_kaitai.NewFixedStruct()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for fixed_struct (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
