...
import (
	...
	// The alias helps avoid name collisions if position_to_end is a common word.
...
func loadKsySchemaForPositionToEnd(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForPositionToEnd(t *testing.T, data any) map[string]any {
...
func TestParse_PositionToEnd(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "position_to_end.ksy")
	ksySchema := loadKsySchemaForPositionToEnd(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("position_to_end_position_to_end_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "position_to_end.bin")
		...
		kscParsed := position_to_end_kaitai.NewPositionToEnd() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for position_to_end (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_PositionToEnd(t *testing.T) {
	...
	t.Run("position_to_end_position_to_end_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "position_to_end.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := position_to_end_kaitai.NewPositionToEnd()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForPositionToEnd(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := position_to_end_kaitai.NewPositionToEnd()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for position_to_end (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
