...
import (
	...
	// The alias helps avoid name collisions if valid_fail_expr is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailExpr(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailExpr(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailExpr(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_expr.ksy")
	ksySchema := loadKsySchemaForValidFailExpr(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_expr, skipping detailed parse tests.")

}

func TestSerialize_ValidFailExpr(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_expr.ksy")
	ksySchema := loadKsySchemaForValidFailExpr(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_expr, skipping detailed serialize tests.")
