...
import (
	...
	valid_fail_expr_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_expr"
)

func TestParse_ValidFailExpr(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_expr.ksy")
	...
	// Read binary file
	binPath := filepath.Join("../../../test/src", "nav_parent_switch.bin")
...
func TestSerialize_ValidFailExpr(t *testing.T) {
	...
	original := valid_fail_expr_kaitai.NewValidFailExpr()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailExpr(t, original)
	...
	reparsed := valid_fail_expr_kaitai.NewValidFailExpr()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_expr_kaitai.ValidFailExpr{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailExpr converts KSC struct to map for serializer
func structToMapValidFailExpr(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailExpr(t, data)
}

// structToMapReflectiveValidFailExpr uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailExpr(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailExpr(t, fieldVal.Interface())
