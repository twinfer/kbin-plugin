...
import (
	...
	// The alias helps avoid name collisions if zlib_surrounded is a common word.
...
func loadKsySchemaForZlibSurrounded(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForZlibSurrounded(t *testing.T, data any) map[string]any {
...
func TestParse_ZlibSurrounded(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "zlib_surrounded.ksy")
	ksySchema := loadKsySchemaForZlibSurrounded(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("zlib_surrounded_zlib_surrounded_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "zlib_surrounded.bin")
		...
		kscParsed := zlib_surrounded_kaitai.NewZlibSurrounded() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for zlib_surrounded (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_ZlibSurrounded(t *testing.T) {
	...
	t.Run("zlib_surrounded_zlib_surrounded_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "zlib_surrounded.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := zlib_surrounded_kaitai.NewZlibSurrounded()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForZlibSurrounded(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := zlib_surrounded_kaitai.NewZlibSurrounded()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for zlib_surrounded (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
