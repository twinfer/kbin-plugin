...
func TestParse_CombineBool(t *testing.T) {
	...
	// Assert BoolBit
	if boolbit_val, ok := customMap["BoolBit"]; ok {
		assert.EqualValues(t, true, boolbit_val)
	} else {
		t.Fatalf("Field 'BoolBit' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert BoolCalcBit
	if boolcalcbit_val, ok := customMap["BoolCalcBit"]; ok {
		assert.EqualValues(t, false, boolcalcbit_val)
	} else {
		t.Fatalf("Field 'BoolCalcBit' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
