...
import (
	...
	// The alias helps avoid name collisions if params_call is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForParamsCall(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForParamsCall(t *testing.T, data any) map[string]any {
...
func TestParse_ParamsCall(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "params_call.ksy")
	ksySchema := loadKsySchemaForParamsCall(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for params_call, skipping detailed parse tests.")
...
func TestSerialize_ParamsCall(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for params_call, skipping detailed serialize tests.")
