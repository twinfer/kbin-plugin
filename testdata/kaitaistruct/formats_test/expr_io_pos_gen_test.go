...
import (
	...
	// The alias helps avoid name collisions if expr_io_pos is a common word.
...
func loadKsySchemaForExprIoPos(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprIoPos(t *testing.T, data any) map[string]any {
...
func TestParse_ExprIoPos(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_io_pos.ksy")
	ksySchema := loadKsySchemaForExprIoPos(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("expr_io_pos_expr_io_pos_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "expr_io_pos.bin")
		...
		kscParsed := expr_io_pos_kaitai.NewExprIoPos() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for expr_io_pos (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_ExprIoPos(t *testing.T) {
	...
	t.Run("expr_io_pos_expr_io_pos_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "expr_io_pos.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := expr_io_pos_kaitai.NewExprIoPos()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForExprIoPos(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := expr_io_pos_kaitai.NewExprIoPos()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for expr_io_pos (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
