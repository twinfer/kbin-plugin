...
import (
	...
	// The alias helps avoid name collisions if floating_points is a common word.
	floating_points_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/floating_points"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForFloatingPoints(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForFloatingPoints(t *testing.T, data any) map[string]any {
...
func TestParse_FloatingPoints(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "floating_points.ksy")
	ksySchema := loadKsySchemaForFloatingPoints(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("floating_points_floating_points_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "floating_points.bin")
		...
		kscParsed := floating_points_kaitai.NewFloatingPoints() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for floating_points (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_FloatingPoints(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "floating_points.ksy")
	ksySchema := loadKsySchemaForFloatingPoints(t, ksyFilePath)
	...
	t.Run("floating_points_floating_points_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "floating_points.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := floating_points_kaitai.NewFloatingPoints()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForFloatingPoints(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := floating_points_kaitai.NewFloatingPoints()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for floating_points (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
