...
func TestParse_FloatingPoints(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "floating_points.ksy")
...
func TestSerialize_FloatingPoints(t *testing.T) {
	...
	original := floating_points_kaitai.NewFloatingPoints()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapFloatingPoints(t, original)
	...
	reparsed := floating_points_kaitai.NewFloatingPoints()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(floating_points_kaitai.FloatingPoints{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapFloatingPoints converts KSC struct to map for serializer
func structToMapFloatingPoints(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveFloatingPoints(t, data)
}

// structToMapReflectiveFloatingPoints uses reflection to convert KSC structs including method calls
func structToMapReflectiveFloatingPoints(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveFloatingPoints(t, fieldVal.Interface())
