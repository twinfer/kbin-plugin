...
import (
	...
	// The alias helps avoid name collisions if index_to_param_eos is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForIndexToParamEos(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForIndexToParamEos(t *testing.T, data any) map[string]any {
...
func TestParse_IndexToParamEos(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "index_to_param_eos.ksy")
	ksySchema := loadKsySchemaForIndexToParamEos(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for index_to_param_eos, skipping detailed parse tests.")

}

func TestSerialize_IndexToParamEos(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "index_to_param_eos.ksy")
	ksySchema := loadKsySchemaForIndexToParamEos(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for index_to_param_eos, skipping detailed serialize tests.")
