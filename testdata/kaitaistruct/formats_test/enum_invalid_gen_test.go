...
import (
	...
	enum_invalid_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_invalid"
)

func TestParse_EnumInvalid(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_invalid.ksy")
	...
	if pet1_val, ok := customMap["Pet1"]; ok {
		assert.EqualValues(t, EnumInvalid_Animal__Dog, pet1_val)
	} else {
...
func TestSerialize_EnumInvalid(t *testing.T) {
	...
	original := enum_invalid_kaitai.NewEnumInvalid()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumInvalid(t, original)
	...
	reparsed := enum_invalid_kaitai.NewEnumInvalid()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_invalid_kaitai.EnumInvalid{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumInvalid converts KSC struct to map for serializer
func structToMapEnumInvalid(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumInvalid(t, data)
}

// structToMapReflectiveEnumInvalid uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumInvalid(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumInvalid(t, fieldVal.Interface())
