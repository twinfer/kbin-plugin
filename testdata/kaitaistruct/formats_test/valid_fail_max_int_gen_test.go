...
import (
	...
	valid_fail_max_int_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_max_int"
)

func TestParse_ValidFailMaxInt(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_max_int.ksy")
...
func TestSerialize_ValidFailMaxInt(t *testing.T) {
	...
	original := valid_fail_max_int_kaitai.NewValidFailMaxInt()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailMaxInt(t, original)
	...
	reparsed := valid_fail_max_int_kaitai.NewValidFailMaxInt()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_max_int_kaitai.ValidFailMaxInt{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailMaxInt converts KSC struct to map for serializer
func structToMapValidFailMaxInt(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailMaxInt(t, data)
}

// structToMapReflectiveValidFailMaxInt uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailMaxInt(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailMaxInt(t, fieldVal.Interface())
