...
import (
	...
	// The alias helps avoid name collisions if process_coerce_bytes is a common word.
	process_coerce_bytes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/process_coerce_bytes"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForProcessCoerceBytes(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForProcessCoerceBytes(t *testing.T, data any) map[string]any {
...
func TestParse_ProcessCoerceBytes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_coerce_bytes.ksy")
	ksySchema := loadKsySchemaForProcessCoerceBytes(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("process_coerce_bytes_process_coerce_bytes_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "process_coerce_bytes.bin")
		...
		kscParsed := process_coerce_bytes_kaitai.NewProcessCoerceBytes() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for process_coerce_bytes (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_ProcessCoerceBytes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_coerce_bytes.ksy")
	ksySchema := loadKsySchemaForProcessCoerceBytes(t, ksyFilePath)
	...
	t.Run("process_coerce_bytes_process_coerce_bytes_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "process_coerce_bytes.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := process_coerce_bytes_kaitai.NewProcessCoerceBytes()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForProcessCoerceBytes(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := process_coerce_bytes_kaitai.NewProcessCoerceBytes()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for process_coerce_bytes (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
