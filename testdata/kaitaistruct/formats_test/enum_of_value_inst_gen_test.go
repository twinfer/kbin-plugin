...
import (
	...
	// The alias helps avoid name collisions if enum_of_value_inst is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumOfValueInst(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumOfValueInst(t *testing.T, data any) map[string]any {
...
func TestParse_EnumOfValueInst(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_of_value_inst.ksy")
	ksySchema := loadKsySchemaForEnumOfValueInst(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for enum_of_value_inst, skipping detailed parse tests.")

}

func TestSerialize_EnumOfValueInst(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_of_value_inst.ksy")
	ksySchema := loadKsySchemaForEnumOfValueInst(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for enum_of_value_inst, skipping detailed serialize tests.")
