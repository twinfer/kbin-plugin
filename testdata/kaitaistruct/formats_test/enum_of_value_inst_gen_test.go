...
import (
	...
	enum_of_value_inst_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_of_value_inst"
)

func TestParse_EnumOfValueInst(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_of_value_inst.ksy")
	...
	if pet1_val, ok := customMap["Pet1"]; ok {
		assert.EqualValues(t, EnumOfValueInst_Animal__Cat, pet1_val)
	} else {
	...
	if pet2_val, ok := customMap["Pet2"]; ok {
		assert.EqualValues(t, EnumOfValueInst_Animal__Chicken, pet2_val)
	} else {
		t.Fatalf("Field 'Pet2' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert Pet3
	if pet3_val, ok := customMap["Pet3"]; ok {
		assert.EqualValues(t, EnumOfValueInst_Animal__Dog, pet3_val)
	} else {
		t.Fatalf("Field 'Pet3' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert Pet4
	if pet4_val, ok := customMap["Pet4"]; ok {
		assert.EqualValues(t, EnumOfValueInst_Animal__Dog, pet4_val)
	} else {
		t.Fatalf("Field 'Pet4' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
...
func TestSerialize_EnumOfValueInst(t *testing.T) {
	...
	original := enum_of_value_inst_kaitai.NewEnumOfValueInst()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumOfValueInst(t, original)
	...
	reparsed := enum_of_value_inst_kaitai.NewEnumOfValueInst()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_of_value_inst_kaitai.EnumOfValueInst{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumOfValueInst converts KSC struct to map for serializer
func structToMapEnumOfValueInst(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumOfValueInst(t, data)
}

// structToMapReflectiveEnumOfValueInst uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumOfValueInst(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumOfValueInst(t, fieldVal.Interface())
