...
import (
	...
	// The alias helps avoid name collisions if nav_parent_false is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForNavParentFalse(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForNavParentFalse(t *testing.T, data any) map[string]any {
...
func TestParse_NavParentFalse(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nav_parent_false.ksy")
	ksySchema := loadKsySchemaForNavParentFalse(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for nav_parent_false, skipping detailed parse tests.")
...
func TestSerialize_NavParentFalse(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for nav_parent_false, skipping detailed serialize tests.")
