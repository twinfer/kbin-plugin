...
import (
	...
	valid_fail_anyof_int_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_anyof_int"
)

func TestParse_ValidFailAnyofInt(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_anyof_int.ksy")
...
func TestSerialize_ValidFailAnyofInt(t *testing.T) {
	...
	original := valid_fail_anyof_int_kaitai.NewValidFailAnyofInt()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailAnyofInt(t, original)
	...
	reparsed := valid_fail_anyof_int_kaitai.NewValidFailAnyofInt()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_anyof_int_kaitai.ValidFailAnyofInt{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailAnyofInt converts KSC struct to map for serializer
func structToMapValidFailAnyofInt(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailAnyofInt(t, data)
}

// structToMapReflectiveValidFailAnyofInt uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailAnyofInt(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailAnyofInt(t, fieldVal.Interface())
