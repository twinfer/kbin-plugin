...
import (
	...
	// The alias helps avoid name collisions if repeat_until_sized is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForRepeatUntilSized(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForRepeatUntilSized(t *testing.T, data any) map[string]any {
...
func TestParse_RepeatUntilSized(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_until_sized.ksy")
	ksySchema := loadKsySchemaForRepeatUntilSized(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for repeat_until_sized, skipping detailed parse tests.")

}

func TestSerialize_RepeatUntilSized(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_until_sized.ksy")
	ksySchema := loadKsySchemaForRepeatUntilSized(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for repeat_until_sized, skipping detailed serialize tests.")
