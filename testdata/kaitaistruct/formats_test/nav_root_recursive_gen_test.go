...
import (
	...
	// The alias helps avoid name collisions if nav_root_recursive is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForNavRootRecursive(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForNavRootRecursive(t *testing.T, data any) map[string]any {
...
func TestParse_NavRootRecursive(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nav_root_recursive.ksy")
	ksySchema := loadKsySchemaForNavRootRecursive(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for nav_root_recursive, skipping detailed parse tests.")
...
func TestSerialize_NavRootRecursive(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for nav_root_recursive, skipping detailed serialize tests.")
