...
import (
	...
	eos_exception_bytes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/eos_exception_bytes"
)

func TestParse_EosExceptionBytes(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "eos_exception_bytes.ksy")
...
func TestSerialize_EosExceptionBytes(t *testing.T) {
	...
	original := eos_exception_bytes_kaitai.NewEosExceptionBytes()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEosExceptionBytes(t, original)
	...
	reparsed := eos_exception_bytes_kaitai.NewEosExceptionBytes()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(eos_exception_bytes_kaitai.EosExceptionBytes{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEosExceptionBytes converts KSC struct to map for serializer
func structToMapEosExceptionBytes(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEosExceptionBytes(t, data)
}

// structToMapReflectiveEosExceptionBytes uses reflection to convert KSC structs including method calls
func structToMapReflectiveEosExceptionBytes(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEosExceptionBytes(t, fieldVal.Interface())
