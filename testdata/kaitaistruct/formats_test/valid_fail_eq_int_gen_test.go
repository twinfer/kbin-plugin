...
import (
	...
	valid_fail_eq_int_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_eq_int"
)

func TestParse_ValidFailEqInt(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_eq_int.ksy")
...
func TestSerialize_ValidFailEqInt(t *testing.T) {
	...
	original := valid_fail_eq_int_kaitai.NewValidFailEqInt()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailEqInt(t, original)
	...
	reparsed := valid_fail_eq_int_kaitai.NewValidFailEqInt()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_eq_int_kaitai.ValidFailEqInt{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailEqInt converts KSC struct to map for serializer
func structToMapValidFailEqInt(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailEqInt(t, data)
}

// structToMapReflectiveValidFailEqInt uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailEqInt(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailEqInt(t, fieldVal.Interface())
