...
import (
	...
	// The alias helps avoid name collisions if valid_fail_eq_int is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailEqInt(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailEqInt(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailEqInt(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_eq_int.ksy")
	ksySchema := loadKsySchemaForValidFailEqInt(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_eq_int, skipping detailed parse tests.")

}

func TestSerialize_ValidFailEqInt(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_eq_int.ksy")
	ksySchema := loadKsySchemaForValidFailEqInt(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_eq_int, skipping detailed serialize tests.")
