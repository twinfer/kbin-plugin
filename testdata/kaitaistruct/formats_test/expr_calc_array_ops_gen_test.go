...
import (
	...
	// The alias helps avoid name collisions if expr_calc_array_ops is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForExprCalcArrayOps(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprCalcArrayOps(t *testing.T, data any) map[string]any {
...
func TestParse_ExprCalcArrayOps(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_calc_array_ops.ksy")
	ksySchema := loadKsySchemaForExprCalcArrayOps(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for expr_calc_array_ops, skipping detailed parse tests.")
...
func TestSerialize_ExprCalcArrayOps(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for expr_calc_array_ops, skipping detailed serialize tests.")
