...
import (
	...
	// The alias helps avoid name collisions if integers_double_overflow is a common word.
	integers_double_overflow_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/integers_double_overflow"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForIntegersDoubleOverflow(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForIntegersDoubleOverflow(t *testing.T, data any) map[string]any {
...
func TestParse_IntegersDoubleOverflow(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "integers_double_overflow.ksy")
	ksySchema := loadKsySchemaForIntegersDoubleOverflow(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("integers_double_overflow_integers_double_overflow_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "integers_double_overflow.bin")
		...
		kscParsed := integers_double_overflow_kaitai.NewIntegersDoubleOverflow() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for integers_double_overflow (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_IntegersDoubleOverflow(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "integers_double_overflow.ksy")
	ksySchema := loadKsySchemaForIntegersDoubleOverflow(t, ksyFilePath)
	...
	t.Run("integers_double_overflow_integers_double_overflow_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "integers_double_overflow.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := integers_double_overflow_kaitai.NewIntegersDoubleOverflow()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForIntegersDoubleOverflow(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := integers_double_overflow_kaitai.NewIntegersDoubleOverflow()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for integers_double_overflow (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
