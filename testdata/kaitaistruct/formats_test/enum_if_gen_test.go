...
import (
	...
	// The alias helps avoid name collisions if enum_if is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumIf(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumIf(t *testing.T, data any) map[string]any {
...
func TestParse_EnumIf(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_if.ksy")
	ksySchema := loadKsySchemaForEnumIf(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for enum_if, skipping detailed parse tests.")
...
func TestSerialize_EnumIf(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for enum_if, skipping detailed serialize tests.")
