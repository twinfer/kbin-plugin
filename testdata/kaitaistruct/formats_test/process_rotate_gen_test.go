...
import (
	...
	// The alias helps avoid name collisions if process_rotate is a common word.
...
func loadKsySchemaForProcessRotate(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForProcessRotate(t *testing.T, data any) map[string]any {
...
func TestParse_ProcessRotate(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_rotate.ksy")
	ksySchema := loadKsySchemaForProcessRotate(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("process_rotate_process_rotate_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "process_rotate.bin")
		...
		kscParsed := process_rotate_kaitai.NewProcessRotate() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for process_rotate (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_ProcessRotate(t *testing.T) {
	...
	t.Run("process_rotate_process_rotate_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "process_rotate.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := process_rotate_kaitai.NewProcessRotate()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForProcessRotate(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := process_rotate_kaitai.NewProcessRotate()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for process_rotate (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
