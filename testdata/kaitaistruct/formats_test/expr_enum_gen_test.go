...
import (
	...
	expr_enum_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/expr_enum"
)

func TestParse_ExprEnum(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "expr_enum.ksy")
	...
	// Assert ConstDog
	if constdog_val, ok := customMap["ConstDog"]; ok {
		assert.EqualValues(t, ExprEnum_Animal__Dog, constdog_val)
	} else {
		t.Fatalf("Field 'ConstDog' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert DerivedBoom
	if derivedboom_val, ok := customMap["DerivedBoom"]; ok {
		assert.EqualValues(t, ExprEnum_Animal__Boom, derivedboom_val)
	} else {
		t.Fatalf("Field 'DerivedBoom' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert DerivedDog
	if deriveddog_val, ok := customMap["DerivedDog"]; ok {
		assert.EqualValues(t, ExprEnum_Animal__Dog, deriveddog_val)
	} else {
		t.Fatalf("Field 'DerivedDog' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
...
func TestSerialize_ExprEnum(t *testing.T) {
	...
	original := expr_enum_kaitai.NewExprEnum()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapExprEnum(t, original)
	...
	reparsed := expr_enum_kaitai.NewExprEnum()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(expr_enum_kaitai.ExprEnum{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapExprEnum converts KSC struct to map for serializer
func structToMapExprEnum(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveExprEnum(t, data)
}

// structToMapReflectiveExprEnum uses reflection to convert KSC structs including method calls
func structToMapReflectiveExprEnum(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveExprEnum(t, fieldVal.Interface())
