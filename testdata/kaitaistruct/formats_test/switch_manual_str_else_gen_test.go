...
import (
	...
	// The alias helps avoid name collisions if switch_manual_str_else is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForSwitchManualStrElse(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForSwitchManualStrElse(t *testing.T, data any) map[string]any {
...
func TestParse_SwitchManualStrElse(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_manual_str_else.ksy")
	ksySchema := loadKsySchemaForSwitchManualStrElse(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for switch_manual_str_else, skipping detailed parse tests.")

}

func TestSerialize_SwitchManualStrElse(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_manual_str_else.ksy")
	ksySchema := loadKsySchemaForSwitchManualStrElse(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for switch_manual_str_else, skipping detailed serialize tests.")
