...
import (
	...
	// The alias helps avoid name collisions if process_coerce_switch is a common word.
	process_coerce_switch_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/process_coerce_switch"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForProcessCoerceSwitch(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForProcessCoerceSwitch(t *testing.T, data any) map[string]any {
...
func TestParse_ProcessCoerceSwitch(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_coerce_switch.ksy")
	ksySchema := loadKsySchemaForProcessCoerceSwitch(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("process_coerce_switch_process_coerce_switch_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "process_coerce_switch.bin")
		...
		kscParsed := process_coerce_switch_kaitai.NewProcessCoerceSwitch() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for process_coerce_switch (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_ProcessCoerceSwitch(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_coerce_switch.ksy")
	ksySchema := loadKsySchemaForProcessCoerceSwitch(t, ksyFilePath)
	...
	t.Run("process_coerce_switch_process_coerce_switch_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "process_coerce_switch.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := process_coerce_switch_kaitai.NewProcessCoerceSwitch()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForProcessCoerceSwitch(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := process_coerce_switch_kaitai.NewProcessCoerceSwitch()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for process_coerce_switch (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
