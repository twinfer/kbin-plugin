...
func TestParse_NestedTypeParam(t *testing.T) {
	...
	if mainseq_mylen_lvl0_map, ok := customMap["MainSeq"].(map[string]any); ok {
		if mainseq_mylen_val, ok := mainseq_mylen_lvl0_map["MyLen"]; ok {
			assert.EqualValues(t, 5, mainseq_mylen_val)
		} else {
			t.Fatalf("Field 'MyLen' not found in mainseq_mylen_lvl0_map (keys: %v)", maps.Keys(mainseq_mylen_lvl0_map))
		}
	} else {
		t.Fatalf("Field 'MainSeq' not found or not a map while asserting MainSeq.MyLen")
	}

	// Assert MainSeq.Body
	if mainseq_body_lvl0_map, ok := customMap["MainSeq"].(map[string]any); ok {
		if mainseq_body_val, ok := mainseq_body_lvl0_map["Body"]; ok {
			assert.EqualValues(t, "foo|b", mainseq_body_val)
		} else {
			t.Fatalf("Field 'Body' not found in mainseq_body_lvl0_map (keys: %v)", maps.Keys(mainseq_body_lvl0_map))
		}
	} else {
		t.Fatalf("Field 'MainSeq' not found or not a map while asserting MainSeq.Body")
	}
