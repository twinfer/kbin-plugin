...
import (
	...
	default_endian_expr_exception_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/default_endian_expr_exception"
)

func TestParse_DefaultEndianExprException(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "default_endian_expr_exception.ksy")
	...
	// Read binary file
	binPath := filepath.Join("../../../test/src", "endian_expr.bin")
...
func TestSerialize_DefaultEndianExprException(t *testing.T) {
	...
	original := default_endian_expr_exception_kaitai.NewDefaultEndianExprException()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapDefaultEndianExprException(t, original)
	...
	reparsed := default_endian_expr_exception_kaitai.NewDefaultEndianExprException()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(default_endian_expr_exception_kaitai.DefaultEndianExprException{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapDefaultEndianExprException converts KSC struct to map for serializer
func structToMapDefaultEndianExprException(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveDefaultEndianExprException(t, data)
}

// structToMapReflectiveDefaultEndianExprException uses reflection to convert KSC structs including method calls
func structToMapReflectiveDefaultEndianExprException(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveDefaultEndianExprException(t, fieldVal.Interface())
