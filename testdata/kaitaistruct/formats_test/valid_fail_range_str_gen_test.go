...
import (
	...
	// The alias helps avoid name collisions if valid_fail_range_str is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailRangeStr(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailRangeStr(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailRangeStr(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_range_str.ksy")
	ksySchema := loadKsySchemaForValidFailRangeStr(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_range_str, skipping detailed parse tests.")
...
func TestSerialize_ValidFailRangeStr(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_range_str, skipping detailed serialize tests.")
