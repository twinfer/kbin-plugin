...
import (
	...
	enum_import_seq_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_import_seq"
)

func TestParse_EnumImportSeq(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_import_seq.ksy")
	...
	if pet1_val, ok := customMap["Pet1"]; ok {
		assert.EqualValues(t, Enum0_Animal__Cat, pet1_val)
	} else {
...
func TestSerialize_EnumImportSeq(t *testing.T) {
	...
	original := enum_import_seq_kaitai.NewEnumImportSeq()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumImportSeq(t, original)
	...
	reparsed := enum_import_seq_kaitai.NewEnumImportSeq()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_import_seq_kaitai.EnumImportSeq{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumImportSeq converts KSC struct to map for serializer
func structToMapEnumImportSeq(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumImportSeq(t, data)
}

// structToMapReflectiveEnumImportSeq uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumImportSeq(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumImportSeq(t, fieldVal.Interface())
