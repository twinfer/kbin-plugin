...
import (
	...
	// The alias helps avoid name collisions if if_struct is a common word.
...
func loadKsySchemaForIfStruct(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForIfStruct(t *testing.T, data any) map[string]any {
...
func TestParse_IfStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "if_struct.ksy")
	ksySchema := loadKsySchemaForIfStruct(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("if_struct_if_struct_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "if_struct.bin")
		...
		kscParsed := if_struct_kaitai.NewIfStruct() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for if_struct (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_IfStruct(t *testing.T) {
	...
	t.Run("if_struct_if_struct_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "if_struct.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := if_struct_kaitai.NewIfStruct()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForIfStruct(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := if_struct_kaitai.NewIfStruct()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for if_struct (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
