...
import (
	...
	// The alias helps avoid name collisions if nav_parent2 is a common word.
	nav_parent2_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/nav_parent2"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForNavParent2(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForNavParent2(t *testing.T, data any) map[string]any {
...
func TestParse_NavParent2(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nav_parent2.ksy")
	ksySchema := loadKsySchemaForNavParent2(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("nav_parent2_nav_parent2_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "nav_parent2.bin")
		...
		kscParsed := nav_parent2_kaitai.NewNavParent2() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for nav_parent2 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_NavParent2(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "nav_parent2.ksy")
	ksySchema := loadKsySchemaForNavParent2(t, ksyFilePath)
	...
	t.Run("nav_parent2_nav_parent2_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "nav_parent2.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := nav_parent2_kaitai.NewNavParent2()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForNavParent2(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := nav_parent2_kaitai.NewNavParent2()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for nav_parent2 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
