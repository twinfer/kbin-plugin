...
import (
	...
	enum_deep_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_deep"
)

func TestParse_EnumDeep(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_deep.ksy")
	...
	if pet1_val, ok := customMap["Pet1"]; ok {
		assert.EqualValues(t, EnumDeep_Container1_Animal__Cat, pet1_val)
	} else {
...
func TestSerialize_EnumDeep(t *testing.T) {
	...
	original := enum_deep_kaitai.NewEnumDeep()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumDeep(t, original)
	...
	reparsed := enum_deep_kaitai.NewEnumDeep()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_deep_kaitai.EnumDeep{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumDeep converts KSC struct to map for serializer
func structToMapEnumDeep(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumDeep(t, data)
}

// structToMapReflectiveEnumDeep uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumDeep(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumDeep(t, fieldVal.Interface())
