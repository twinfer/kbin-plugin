...
import (
	...
	eof_exception_sized_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/eof_exception_sized"
)

func TestParse_EofExceptionSized(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "eof_exception_sized.ksy")
...
func TestSerialize_EofExceptionSized(t *testing.T) {
	...
	original := eof_exception_sized_kaitai.NewEofExceptionSized()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEofExceptionSized(t, original)
	...
	reparsed := eof_exception_sized_kaitai.NewEofExceptionSized()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(eof_exception_sized_kaitai.EofExceptionSized{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEofExceptionSized converts KSC struct to map for serializer
func structToMapEofExceptionSized(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEofExceptionSized(t, data)
}

// structToMapReflectiveEofExceptionSized uses reflection to convert KSC structs including method calls
func structToMapReflectiveEofExceptionSized(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEofExceptionSized(t, fieldVal.Interface())
