...
import (
	...
	// The alias helps avoid name collisions if if_values is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForIfValues(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForIfValues(t *testing.T, data any) map[string]any {
...
func TestParse_IfValues(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "if_values.ksy")
	ksySchema := loadKsySchemaForIfValues(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for if_values, skipping detailed parse tests.")

}

func TestSerialize_IfValues(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "if_values.ksy")
	ksySchema := loadKsySchemaForIfValues(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for if_values, skipping detailed serialize tests.")
