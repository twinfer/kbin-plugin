...
import (
	...
	// The alias helps avoid name collisions if enum_0 is a common word.
	enum_0_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_0"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnum0(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnum0(t *testing.T, data any) map[string]any {
...
func TestParse_Enum0(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_0.ksy")
	ksySchema := loadKsySchemaForEnum0(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("enum_0_enum_0_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_0.bin")
		...
		kscParsed := enum_0_kaitai.NewEnum0() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for enum_0 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_Enum0(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_0.ksy")
	ksySchema := loadKsySchemaForEnum0(t, ksyFilePath)
	...
	t.Run("enum_0_enum_0_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_0.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := enum_0_kaitai.NewEnum0()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForEnum0(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := enum_0_kaitai.NewEnum0()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for enum_0 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
