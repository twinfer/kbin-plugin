...
import (
	...
	valid_fail_eq_str_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_eq_str"
)

func TestParse_ValidFailEqStr(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_eq_str.ksy")
...
func TestSerialize_ValidFailEqStr(t *testing.T) {
	...
	original := valid_fail_eq_str_kaitai.NewValidFailEqStr()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailEqStr(t, original)
	...
	reparsed := valid_fail_eq_str_kaitai.NewValidFailEqStr()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_eq_str_kaitai.ValidFailEqStr{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailEqStr converts KSC struct to map for serializer
func structToMapValidFailEqStr(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailEqStr(t, data)
}

// structToMapReflectiveValidFailEqStr uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailEqStr(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailEqStr(t, fieldVal.Interface())
