...
import (
	...
	// The alias helps avoid name collisions if valid_fail_eq_str is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailEqStr(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailEqStr(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailEqStr(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_eq_str.ksy")
	ksySchema := loadKsySchemaForValidFailEqStr(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_eq_str, skipping detailed parse tests.")

}

func TestSerialize_ValidFailEqStr(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_eq_str.ksy")
	ksySchema := loadKsySchemaForValidFailEqStr(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_eq_str, skipping detailed serialize tests.")
