...
import (
	...
	params_enum_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/params_enum"
)

func TestParse_ParamsEnum(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "params_enum.ksy")
	...
	if one_val, ok := customMap["One"]; ok {
		assert.EqualValues(t, ParamsEnum_Animal__Cat, one_val)
	} else {
		t.Fatalf("Field 'One' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert InvokeWithParam.IsCat
	if invokewithparam_iscat_lvl0_map, ok := customMap["InvokeWithParam"].(map[string]any); ok {
		if invokewithparam_iscat_val, ok := invokewithparam_iscat_lvl0_map["IsCat"]; ok {
			assert.EqualValues(t, true, invokewithparam_iscat_val)
		} else {
			t.Fatalf("Field 'IsCat' not found in invokewithparam_iscat_lvl0_map (keys: %v)", maps.Keys(invokewithparam_iscat_lvl0_map))
		}
	} else {
		t.Fatalf("Field 'InvokeWithParam' not found or not a map while asserting InvokeWithParam.IsCat")
	}
...
func TestSerialize_ParamsEnum(t *testing.T) {
	...
	original := params_enum_kaitai.NewParamsEnum()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapParamsEnum(t, original)
	...
	reparsed := params_enum_kaitai.NewParamsEnum()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(params_enum_kaitai.ParamsEnum{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapParamsEnum converts KSC struct to map for serializer
func structToMapParamsEnum(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveParamsEnum(t, data)
}

// structToMapReflectiveParamsEnum uses reflection to convert KSC structs including method calls
func structToMapReflectiveParamsEnum(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveParamsEnum(t, fieldVal.Interface())
