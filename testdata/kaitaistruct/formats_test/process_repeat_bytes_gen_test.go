...
import (
	...
	process_repeat_bytes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/process_repeat_bytes"
)

func TestParse_ProcessRepeatBytes(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "process_repeat_bytes.ksy")
...
func TestSerialize_ProcessRepeatBytes(t *testing.T) {
	...
	original := process_repeat_bytes_kaitai.NewProcessRepeatBytes()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapProcessRepeatBytes(t, original)
	...
	reparsed := process_repeat_bytes_kaitai.NewProcessRepeatBytes()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(process_repeat_bytes_kaitai.ProcessRepeatBytes{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapProcessRepeatBytes converts KSC struct to map for serializer
func structToMapProcessRepeatBytes(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveProcessRepeatBytes(t, data)
}

// structToMapReflectiveProcessRepeatBytes uses reflection to convert KSC structs including method calls
func structToMapReflectiveProcessRepeatBytes(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveProcessRepeatBytes(t, fieldVal.Interface())
