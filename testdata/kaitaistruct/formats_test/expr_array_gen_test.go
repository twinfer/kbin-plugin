...
import (
	...
	// The alias helps avoid name collisions if expr_array is a common word.
...
func loadKsySchemaForExprArray(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprArray(t *testing.T, data any) map[string]any {
...
func TestParse_ExprArray(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_array.ksy")
	ksySchema := loadKsySchemaForExprArray(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("expr_array_expr_array_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "expr_array.bin")
		...
		kscParsed := expr_array_kaitai.NewExprArray() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for expr_array (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_ExprArray(t *testing.T) {
	...
	t.Run("expr_array_expr_array_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "expr_array.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := expr_array_kaitai.NewExprArray()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForExprArray(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := expr_array_kaitai.NewExprArray()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for expr_array (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
