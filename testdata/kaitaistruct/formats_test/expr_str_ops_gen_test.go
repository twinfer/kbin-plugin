...
import (
	...
	// The alias helps avoid name collisions if expr_str_ops is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForExprStrOps(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprStrOps(t *testing.T, data any) map[string]any {
...
func TestParse_ExprStrOps(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_str_ops.ksy")
	ksySchema := loadKsySchemaForExprStrOps(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for expr_str_ops, skipping detailed parse tests.")
...
func TestSerialize_ExprStrOps(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for expr_str_ops, skipping detailed serialize tests.")
