...
import (
	...
	// The alias helps avoid name collisions if bits_seq_endian_combo is a common word.
)

func loadKsySchemaForBitsSeqEndianCombo(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForBitsSeqEndianCombo(t *testing.T, data any) map[string]any {
...
func TestParse_BitsSeqEndianCombo(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bits_seq_endian_combo.ksy")
	ksySchema := loadKsySchemaForBitsSeqEndianCombo(t, ksyPath)
	...
	_ = interpreter
	t.Logf("No binary test cases for bits_seq_endian_combo, specific value assertions might still run if extracted.")
...
func TestSerialize_BitsSeqEndianCombo(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for bits_seq_endian_combo, skipping detailed serialize tests.")
