...
func TestParse_ExprMod(t *testing.T) {
	...
	// Assert ModPosConst
	if modposconst_val, ok := customMap["ModPosConst"]; ok {
		assert.EqualValues(t, 9, modposconst_val)
	} else {
		t.Fatalf("Field 'ModPosConst' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert ModNegConst
	if modnegconst_val, ok := customMap["ModNegConst"]; ok {
		assert.EqualValues(t, 4, modnegconst_val)
	} else {
		t.Fatalf("Field 'ModNegConst' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert ModPosSeq
	if modposseq_val, ok := customMap["ModPosSeq"]; ok {
		assert.EqualValues(t, 5, modposseq_val)
	} else {
		t.Fatalf("Field 'ModPosSeq' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert ModNegSeq
	if modnegseq_val, ok := customMap["ModNegSeq"]; ok {
		assert.EqualValues(t, 2, modnegseq_val)
	} else {
		t.Fatalf("Field 'ModNegSeq' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
