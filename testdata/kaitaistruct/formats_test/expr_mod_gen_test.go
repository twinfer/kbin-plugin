...
import (
	...
	// The alias helps avoid name collisions if expr_mod is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForExprMod(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprMod(t *testing.T, data any) map[string]any {
...
func TestParse_ExprMod(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_mod.ksy")
	ksySchema := loadKsySchemaForExprMod(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for expr_mod, skipping detailed parse tests.")
...
func TestSerialize_ExprMod(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for expr_mod, skipping detailed serialize tests.")
