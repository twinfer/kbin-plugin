...
import (
	...
	// The alias helps avoid name collisions if instance_std_array is a common word.
	instance_std_array_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/instance_std_array"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForInstanceStdArray(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForInstanceStdArray(t *testing.T, data any) map[string]any {
...
func TestParse_InstanceStdArray(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "instance_std_array.ksy")
	ksySchema := loadKsySchemaForInstanceStdArray(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("instance_std_array_instance_std_array_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "instance_std_array.bin")
		...
		kscParsed := instance_std_array_kaitai.NewInstanceStdArray() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for instance_std_array (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_InstanceStdArray(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "instance_std_array.ksy")
	ksySchema := loadKsySchemaForInstanceStdArray(t, ksyFilePath)
	...
	t.Run("instance_std_array_instance_std_array_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "instance_std_array.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := instance_std_array_kaitai.NewInstanceStdArray()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForInstanceStdArray(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := instance_std_array_kaitai.NewInstanceStdArray()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for instance_std_array (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
