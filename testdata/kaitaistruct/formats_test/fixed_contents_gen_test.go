...
import (
	...
	// The alias helps avoid name collisions if fixed_contents is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForFixedContents(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForFixedContents(t *testing.T, data any) map[string]any {
...
func TestParse_FixedContents(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "fixed_contents.ksy")
	ksySchema := loadKsySchemaForFixedContents(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for fixed_contents, skipping detailed parse tests.")

}

func TestSerialize_FixedContents(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "fixed_contents.ksy")
	ksySchema := loadKsySchemaForFixedContents(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for fixed_contents, skipping detailed serialize tests.")
