...
import (
	...
	fixed_contents_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/fixed_contents"
)

func TestParse_FixedContents(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "fixed_contents.ksy")
...
func TestSerialize_FixedContents(t *testing.T) {
	...
	original := fixed_contents_kaitai.NewFixedContents()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapFixedContents(t, original)
	...
	reparsed := fixed_contents_kaitai.NewFixedContents()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(fixed_contents_kaitai.FixedContents{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapFixedContents converts KSC struct to map for serializer
func structToMapFixedContents(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveFixedContents(t, data)
}

// structToMapReflectiveFixedContents uses reflection to convert KSC structs including method calls
func structToMapReflectiveFixedContents(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveFixedContents(t, fieldVal.Interface())
