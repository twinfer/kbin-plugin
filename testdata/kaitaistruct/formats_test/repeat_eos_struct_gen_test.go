...
import (
	...
	// The alias helps avoid name collisions if repeat_eos_struct is a common word.
	repeat_eos_struct_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/repeat_eos_struct"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForRepeatEosStruct(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForRepeatEosStruct(t *testing.T, data any) map[string]any {
...
func TestParse_RepeatEosStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_eos_struct.ksy")
	ksySchema := loadKsySchemaForRepeatEosStruct(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("repeat_eos_struct_repeat_eos_struct_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_eos_struct.bin")
		...
		kscParsed := repeat_eos_struct_kaitai.NewRepeatEosStruct() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for repeat_eos_struct (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_RepeatEosStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_eos_struct.ksy")
	ksySchema := loadKsySchemaForRepeatEosStruct(t, ksyFilePath)
	...
	t.Run("repeat_eos_struct_repeat_eos_struct_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_eos_struct.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := repeat_eos_struct_kaitai.NewRepeatEosStruct()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForRepeatEosStruct(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := repeat_eos_struct_kaitai.NewRepeatEosStruct()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for repeat_eos_struct (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
