...
func TestParse_EnumIntRangeS(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_int_range_s.ksy")
	...
	// Read binary file
	binPath := filepath.Join("../../../test/src", "enum_int_range_s.bin")
	...
	if f1_val, ok := customMap["F1"]; ok {
		assert.EqualValues(t, EnumIntRangeS_Constants__IntMin, f1_val)
	} else {
	...
	if f2_val, ok := customMap["F2"]; ok {
		assert.EqualValues(t, EnumIntRangeS_Constants__Zero, f2_val)
	} else {
	...
	if f3_val, ok := customMap["F3"]; ok {
		assert.EqualValues(t, EnumIntRangeS_Constants__IntMax, f3_val)
	} else {
...
func TestSerialize_EnumIntRangeS(t *testing.T) {
	...
	original := enum_int_range_s_kaitai.NewEnumIntRangeS()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumIntRangeS(t, original)
	...
	reparsed := enum_int_range_s_kaitai.NewEnumIntRangeS()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_int_range_s_kaitai.EnumIntRangeS{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumIntRangeS converts KSC struct to map for serializer
func structToMapEnumIntRangeS(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumIntRangeS(t, data)
}

// structToMapReflectiveEnumIntRangeS uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumIntRangeS(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumIntRangeS(t, fieldVal.Interface())
