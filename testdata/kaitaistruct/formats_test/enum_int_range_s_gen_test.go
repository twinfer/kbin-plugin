...
import (
	...
	// The alias helps avoid name collisions if enum_int_range_s is a common word.
	enum_int_range_s_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_int_range_s"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumIntRangeS(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumIntRangeS(t *testing.T, data any) map[string]any {
...
func TestParse_EnumIntRangeS(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_int_range_s.ksy")
	ksySchema := loadKsySchemaForEnumIntRangeS(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("enum_int_range_s_enum_int_range_s_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_int_range_s.bin")
		...
		kscParsed := enum_int_range_s_kaitai.NewEnumIntRangeS() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for enum_int_range_s (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_EnumIntRangeS(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_int_range_s.ksy")
	ksySchema := loadKsySchemaForEnumIntRangeS(t, ksyFilePath)
	...
	t.Run("enum_int_range_s_enum_int_range_s_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_int_range_s.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := enum_int_range_s_kaitai.NewEnumIntRangeS()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForEnumIntRangeS(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := enum_int_range_s_kaitai.NewEnumIntRangeS()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for enum_int_range_s (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
