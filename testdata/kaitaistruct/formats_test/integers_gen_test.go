...
import (
	...
	// The alias helps avoid name collisions if integers is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForIntegers(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForIntegers(t *testing.T, data any) map[string]any {
...
func TestParse_Integers(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "integers.ksy")
	ksySchema := loadKsySchemaForIntegers(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for integers, skipping detailed parse tests.")
...
func TestSerialize_Integers(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for integers, skipping detailed serialize tests.")
