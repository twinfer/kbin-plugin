...
func TestParse_ExprIfIntOps(t *testing.T) {
	...
	// Assert IsEqPrim
	if iseqprim_val, ok := customMap["IsEqPrim"]; ok {
		assert.EqualValues(t, true, iseqprim_val)
	} else {
		t.Fatalf("Field 'IsEqPrim' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert IsEqBoxed
	if iseqboxed_val, ok := customMap["IsEqBoxed"]; ok {
		assert.EqualValues(t, true, iseqboxed_val)
	} else {
		t.Fatalf("Field 'IsEqBoxed' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
