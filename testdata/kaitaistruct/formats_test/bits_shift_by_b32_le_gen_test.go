...
import (
	...
	// The alias helps avoid name collisions if bits_shift_by_b32_le is a common word.
	bits_shift_by_b32_le_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/bits_shift_by_b32_le"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForBitsShiftByB32Le(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForBitsShiftByB32Le(t *testing.T, data any) map[string]any {
...
func TestParse_BitsShiftByB32Le(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bits_shift_by_b32_le.ksy")
	ksySchema := loadKsySchemaForBitsShiftByB32Le(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("bits_shift_by_b32_le_bits_shift_by_b32_le_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "bits_shift_by_b32_le.bin")
		...
		kscParsed := bits_shift_by_b32_le_kaitai.NewBitsShiftByB32Le() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for bits_shift_by_b32_le (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_BitsShiftByB32Le(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bits_shift_by_b32_le.ksy")
	ksySchema := loadKsySchemaForBitsShiftByB32Le(t, ksyFilePath)
	...
	t.Run("bits_shift_by_b32_le_bits_shift_by_b32_le_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "bits_shift_by_b32_le.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := bits_shift_by_b32_le_kaitai.NewBitsShiftByB32Le()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForBitsShiftByB32Le(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := bits_shift_by_b32_le_kaitai.NewBitsShiftByB32Le()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for bits_shift_by_b32_le (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
