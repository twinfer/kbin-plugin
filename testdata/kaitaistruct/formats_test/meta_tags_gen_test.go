...
import (
	...
	meta_tags_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/meta_tags"
)

func TestParse_MetaTags(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "meta_tags.ksy")
...
func TestSerialize_MetaTags(t *testing.T) {
	...
	original := meta_tags_kaitai.NewMetaTags()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapMetaTags(t, original)
	...
	reparsed := meta_tags_kaitai.NewMetaTags()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(meta_tags_kaitai.MetaTags{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapMetaTags converts KSC struct to map for serializer
func structToMapMetaTags(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveMetaTags(t, data)
}

// structToMapReflectiveMetaTags uses reflection to convert KSC structs including method calls
func structToMapReflectiveMetaTags(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveMetaTags(t, fieldVal.Interface())
