...
func TestParse_BitsByteAligned(t *testing.T) {
	...
	if byte1_val, ok := customMap["Byte1"]; ok {
		assert.EqualValues(t, 65, byte1_val)
	} else {
	...
	if byte2_val, ok := customMap["Byte2"]; ok {
		assert.EqualValues(t, 75, byte2_val)
	} else {
	...
	// Assert Byte3
	if byte3_val, ok := customMap["Byte3"]; ok {
		assert.EqualValues(t, []uint8{255}, byte3_val)
	} else {
		t.Fatalf("Field 'Byte3' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert FullByte
	if fullbyte_val, ok := customMap["FullByte"]; ok {
		assert.EqualValues(t, 255, fullbyte_val)
	} else {
		t.Fatalf("Field 'FullByte' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert Byte4
	if byte4_val, ok := customMap["Byte4"]; ok {
		assert.EqualValues(t, 80, byte4_val)
	} else {
		t.Fatalf("Field 'Byte4' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
