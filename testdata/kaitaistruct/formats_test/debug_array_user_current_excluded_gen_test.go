...
import (
	...
	// The alias helps avoid name collisions if debug_array_user_current_excluded is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForDebugArrayUserCurrentExcluded(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForDebugArrayUserCurrentExcluded(t *testing.T, data any) map[string]any {
...
func TestParse_DebugArrayUserCurrentExcluded(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "debug_array_user_current_excluded.ksy")
	ksySchema := loadKsySchemaForDebugArrayUserCurrentExcluded(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for debug_array_user_current_excluded, skipping detailed parse tests.")

}

func TestSerialize_DebugArrayUserCurrentExcluded(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "debug_array_user_current_excluded.ksy")
	ksySchema := loadKsySchemaForDebugArrayUserCurrentExcluded(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for debug_array_user_current_excluded, skipping detailed serialize tests.")
