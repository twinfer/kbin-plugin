...
import (
	...
	// The alias helps avoid name collisions if zlib_with_header_78 is a common word.
...
func loadKsySchemaForZlibWithHeader78(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForZlibWithHeader78(t *testing.T, data any) map[string]any {
...
func TestParse_ZlibWithHeader78(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "zlib_with_header_78.ksy")
	ksySchema := loadKsySchemaForZlibWithHeader78(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("zlib_with_header_78_zlib_with_header_78_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "zlib_with_header_78.bin")
		...
		kscParsed := zlib_with_header_78_kaitai.NewZlibWithHeader78() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for zlib_with_header_78 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_ZlibWithHeader78(t *testing.T) {
	...
	t.Run("zlib_with_header_78_zlib_with_header_78_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "zlib_with_header_78.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := zlib_with_header_78_kaitai.NewZlibWithHeader78()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForZlibWithHeader78(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := zlib_with_header_78_kaitai.NewZlibWithHeader78()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for zlib_with_header_78 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
