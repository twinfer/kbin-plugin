...
import (
	...
	// The alias helps avoid name collisions if str_pad_term_utf16 is a common word.
...
func loadKsySchemaForStrPadTermUtf16(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForStrPadTermUtf16(t *testing.T, data any) map[string]any {
...
func TestParse_StrPadTermUtf16(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_pad_term_utf16.ksy")
	ksySchema := loadKsySchemaForStrPadTermUtf16(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("str_pad_term_utf16_str_pad_term_utf16_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_pad_term_utf16.bin")
		...
		kscParsed := str_pad_term_utf16_kaitai.NewStrPadTermUtf16() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for str_pad_term_utf16 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_StrPadTermUtf16(t *testing.T) {
	...
	t.Run("str_pad_term_utf16_str_pad_term_utf16_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_pad_term_utf16.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := str_pad_term_utf16_kaitai.NewStrPadTermUtf16()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForStrPadTermUtf16(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := str_pad_term_utf16_kaitai.NewStrPadTermUtf16()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for str_pad_term_utf16 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
