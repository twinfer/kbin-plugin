...
import (
	...
	valid_fail_inst_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_inst"
)

func TestParse_ValidFailInst(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_inst.ksy")
...
func TestSerialize_ValidFailInst(t *testing.T) {
	...
	original := valid_fail_inst_kaitai.NewValidFailInst()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailInst(t, original)
	...
	reparsed := valid_fail_inst_kaitai.NewValidFailInst()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_inst_kaitai.ValidFailInst{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailInst converts KSC struct to map for serializer
func structToMapValidFailInst(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailInst(t, data)
}

// structToMapReflectiveValidFailInst uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailInst(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailInst(t, fieldVal.Interface())
