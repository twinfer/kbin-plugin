...
import (
	...
	// The alias helps avoid name collisions if valid_fail_inst is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForValidFailInst(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForValidFailInst(t *testing.T, data any) map[string]any {
...
func TestParse_ValidFailInst(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_inst.ksy")
	ksySchema := loadKsySchemaForValidFailInst(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for valid_fail_inst, skipping detailed parse tests.")

}

func TestSerialize_ValidFailInst(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "valid_fail_inst.ksy")
	ksySchema := loadKsySchemaForValidFailInst(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for valid_fail_inst, skipping detailed serialize tests.")
