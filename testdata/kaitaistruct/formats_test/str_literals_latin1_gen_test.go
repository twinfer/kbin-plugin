...
import (
	...
	// The alias helps avoid name collisions if str_literals_latin1 is a common word.
...
func loadKsySchemaForStrLiteralsLatin1(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForStrLiteralsLatin1(t *testing.T, data any) map[string]any {
...
func TestParse_StrLiteralsLatin1(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "str_literals_latin1.ksy")
	ksySchema := loadKsySchemaForStrLiteralsLatin1(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("str_literals_latin1_str_literals_latin1_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_literals_latin1.bin")
		...
		kscParsed := str_literals_latin1_kaitai.NewStrLiteralsLatin1() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for str_literals_latin1 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
...
func TestSerialize_StrLiteralsLatin1(t *testing.T) {
	...
	t.Run("str_literals_latin1_str_literals_latin1_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "str_literals_latin1.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := str_literals_latin1_kaitai.NewStrLiteralsLatin1()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForStrLiteralsLatin1(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := str_literals_latin1_kaitai.NewStrLiteralsLatin1()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for str_literals_latin1 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
