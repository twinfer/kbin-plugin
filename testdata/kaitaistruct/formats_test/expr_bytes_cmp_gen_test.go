...
import (
	...
	// The alias helps avoid name collisions if expr_bytes_cmp is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForExprBytesCmp(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForExprBytesCmp(t *testing.T, data any) map[string]any {
...
func TestParse_ExprBytesCmp(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "expr_bytes_cmp.ksy")
	ksySchema := loadKsySchemaForExprBytesCmp(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for expr_bytes_cmp, skipping detailed parse tests.")
...
func TestSerialize_ExprBytesCmp(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for expr_bytes_cmp, skipping detailed serialize tests.")
