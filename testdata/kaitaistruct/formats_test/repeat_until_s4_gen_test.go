...
import (
	...
	// The alias helps avoid name collisions if repeat_until_s4 is a common word.
	repeat_until_s4_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/repeat_until_s4"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForRepeatUntilS4(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForRepeatUntilS4(t *testing.T, data any) map[string]any {
...
func TestParse_RepeatUntilS4(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_until_s4.ksy")
	ksySchema := loadKsySchemaForRepeatUntilS4(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("repeat_until_s4_repeat_until_s4_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_until_s4.bin")
		...
		kscParsed := repeat_until_s4_kaitai.NewRepeatUntilS4() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for repeat_until_s4 (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_RepeatUntilS4(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_until_s4.ksy")
	ksySchema := loadKsySchemaForRepeatUntilS4(t, ksyFilePath)
	...
	t.Run("repeat_until_s4_repeat_until_s4_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_until_s4.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := repeat_until_s4_kaitai.NewRepeatUntilS4()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForRepeatUntilS4(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := repeat_until_s4_kaitai.NewRepeatUntilS4()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for repeat_until_s4 (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
