...
import (
	...
	params_def_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/params_def"
)

func TestParse_ParamsDef(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "params_def.ksy")
	...
	if buf_val, ok := customMap["Buf"]; ok {
		assert.EqualValues(t, "foo|b", buf_val)
	} else {
		t.Fatalf("Field 'Buf' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert Trailer
	if trailer_val, ok := customMap["Trailer"]; ok {
		assert.EqualValues(t, 97, trailer_val)
	} else {
		t.Fatalf("Field 'Trailer' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
...
func TestSerialize_ParamsDef(t *testing.T) {
	...
	original := params_def_kaitai.NewParamsDef()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapParamsDef(t, original)
	...
	reparsed := params_def_kaitai.NewParamsDef()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(params_def_kaitai.ParamsDef{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapParamsDef converts KSC struct to map for serializer
func structToMapParamsDef(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveParamsDef(t, data)
}

// structToMapReflectiveParamsDef uses reflection to convert KSC structs including method calls
func structToMapReflectiveParamsDef(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveParamsDef(t, fieldVal.Interface())
