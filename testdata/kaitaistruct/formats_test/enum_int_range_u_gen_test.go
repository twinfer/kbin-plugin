...
func TestParse_EnumIntRangeU(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_int_range_u.ksy")
	...
	// Read binary file
	binPath := filepath.Join("../../../test/src", "enum_int_range_u.bin")
	...
	if f1_val, ok := customMap["F1"]; ok {
		assert.EqualValues(t, EnumIntRangeU_Constants__Zero, f1_val)
	} else {
	...
	if f2_val, ok := customMap["F2"]; ok {
		assert.EqualValues(t, EnumIntRangeU_Constants__IntMax, f2_val)
	} else {
...
func TestSerialize_EnumIntRangeU(t *testing.T) {
	...
	original := enum_int_range_u_kaitai.NewEnumIntRangeU()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumIntRangeU(t, original)
	...
	reparsed := enum_int_range_u_kaitai.NewEnumIntRangeU()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_int_range_u_kaitai.EnumIntRangeU{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumIntRangeU converts KSC struct to map for serializer
func structToMapEnumIntRangeU(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumIntRangeU(t, data)
}

// structToMapReflectiveEnumIntRangeU uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumIntRangeU(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumIntRangeU(t, fieldVal.Interface())
