...
import (
	...
	// The alias helps avoid name collisions if enum_int_range_u is a common word.
	enum_int_range_u_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_int_range_u"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumIntRangeU(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumIntRangeU(t *testing.T, data any) map[string]any {
...
func TestParse_EnumIntRangeU(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_int_range_u.ksy")
	ksySchema := loadKsySchemaForEnumIntRangeU(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("enum_int_range_u_enum_int_range_u_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_int_range_u.bin")
		...
		kscParsed := enum_int_range_u_kaitai.NewEnumIntRangeU() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for enum_int_range_u (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_EnumIntRangeU(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_int_range_u.ksy")
	ksySchema := loadKsySchemaForEnumIntRangeU(t, ksyFilePath)
	...
	t.Run("enum_int_range_u_enum_int_range_u_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_int_range_u.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := enum_int_range_u_kaitai.NewEnumIntRangeU()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForEnumIntRangeU(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := enum_int_range_u_kaitai.NewEnumIntRangeU()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for enum_int_range_u (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
