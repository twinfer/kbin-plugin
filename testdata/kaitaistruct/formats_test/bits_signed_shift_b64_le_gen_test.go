...
import (
	...
	// The alias helps avoid name collisions if bits_signed_shift_b64_le is a common word.
	bits_signed_shift_b64_le_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/bits_signed_shift_b64_le"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForBitsSignedShiftB64Le(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForBitsSignedShiftB64Le(t *testing.T, data any) map[string]any {
...
func TestParse_BitsSignedShiftB64Le(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bits_signed_shift_b64_le.ksy")
	ksySchema := loadKsySchemaForBitsSignedShiftB64Le(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("bits_signed_shift_b64_le_bits_signed_shift_b64_le_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "bits_signed_shift_b64_le.bin")
		...
		kscParsed := bits_signed_shift_b64_le_kaitai.NewBitsSignedShiftB64Le() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for bits_signed_shift_b64_le (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_BitsSignedShiftB64Le(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bits_signed_shift_b64_le.ksy")
	ksySchema := loadKsySchemaForBitsSignedShiftB64Le(t, ksyFilePath)
	...
	t.Run("bits_signed_shift_b64_le_bits_signed_shift_b64_le_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "bits_signed_shift_b64_le.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := bits_signed_shift_b64_le_kaitai.NewBitsSignedShiftB64Le()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForBitsSignedShiftB64Le(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := bits_signed_shift_b64_le_kaitai.NewBitsSignedShiftB64Le()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for bits_signed_shift_b64_le (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
