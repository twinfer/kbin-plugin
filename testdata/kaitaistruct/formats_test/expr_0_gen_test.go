...
func TestParse_Expr0(t *testing.T) {
	...
	// Assert MustBeF7
	if mustbef7_val, ok := customMap["MustBeF7"]; ok {
		assert.EqualValues(t, 247, mustbef7_val)
	} else {
		t.Fatalf("Field 'MustBeF7' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert MustBeAbc123
	if mustbeabc123_val, ok := customMap["MustBeAbc123"]; ok {
		assert.EqualValues(t, "abc123", mustbeabc123_val)
	} else {
		t.Fatalf("Field 'MustBeAbc123' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
