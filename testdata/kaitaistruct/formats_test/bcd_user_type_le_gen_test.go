...
func TestParse_BcdUserTypeLe(t *testing.T) {
	...
	ksySchema := loadKsySchemaForBcdUserTypeLe(t, ksyFilePath)
	...
	t.Run("bcd_user_type_le_bcd_user_type_le_Parse", func(t *testing.T) {
		...
		kscParsed := bcd_user_type_le_kaitai.NewBcdUserTypeLe() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for bcd_user_type_le (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
