...
import (
	...
	// The alias helps avoid name collisions if combine_str is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForCombineStr(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForCombineStr(t *testing.T, data any) map[string]any {
...
func TestParse_CombineStr(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "combine_str.ksy")
	ksySchema := loadKsySchemaForCombineStr(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for combine_str, skipping detailed parse tests.")
...
func TestSerialize_CombineStr(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for combine_str, skipping detailed serialize tests.")
