...
import (
	...
	// The alias helps avoid name collisions if default_endian_mod is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForDefaultEndianMod(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForDefaultEndianMod(t *testing.T, data any) map[string]any {
...
func TestParse_DefaultEndianMod(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "default_endian_mod.ksy")
	ksySchema := loadKsySchemaForDefaultEndianMod(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for default_endian_mod, skipping detailed parse tests.")
...
func TestSerialize_DefaultEndianMod(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for default_endian_mod, skipping detailed serialize tests.")
