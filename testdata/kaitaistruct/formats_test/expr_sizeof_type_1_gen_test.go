...
func TestParse_ExprSizeofType1(t *testing.T) {
	...
	if sizeofblock_val, ok := customMap["SizeofBlock"]; ok {
		assert.EqualValues(t, (((1 + 4) + 2) + 4), sizeofblock_val)
	} else {
		t.Fatalf("Field 'SizeofBlock' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert SizeofSubblock
	if sizeofsubblock_val, ok := customMap["SizeofSubblock"]; ok {
		assert.EqualValues(t, 4, sizeofsubblock_val)
	} else {
		t.Fatalf("Field 'SizeofSubblock' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
