...
import (
	...
	// The alias helps avoid name collisions if enum_long_range_u is a common word.
	enum_long_range_u_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_long_range_u"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumLongRangeU(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumLongRangeU(t *testing.T, data any) map[string]any {
...
func TestParse_EnumLongRangeU(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_long_range_u.ksy")
	ksySchema := loadKsySchemaForEnumLongRangeU(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("enum_long_range_u_enum_long_range_u_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_long_range_u.bin")
		...
		kscParsed := enum_long_range_u_kaitai.NewEnumLongRangeU() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for enum_long_range_u (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_EnumLongRangeU(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_long_range_u.ksy")
	ksySchema := loadKsySchemaForEnumLongRangeU(t, ksyFilePath)
	...
	t.Run("enum_long_range_u_enum_long_range_u_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_long_range_u.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := enum_long_range_u_kaitai.NewEnumLongRangeU()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForEnumLongRangeU(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := enum_long_range_u_kaitai.NewEnumLongRangeU()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for enum_long_range_u (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
