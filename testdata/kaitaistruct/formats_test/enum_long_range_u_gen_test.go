...
func TestParse_EnumLongRangeU(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_long_range_u.ksy")
	...
	// Read binary file
	binPath := filepath.Join("../../../test/src", "enum_long_range_u.bin")
	...
	if f1_val, ok := customMap["F1"]; ok {
		assert.EqualValues(t, EnumLongRangeU_Constants__Zero, f1_val)
	} else {
	...
	if f2_val, ok := customMap["F2"]; ok {
		assert.EqualValues(t, EnumLongRangeU_Constants__IntMax, f2_val)
	} else {
	...
	if f3_val, ok := customMap["F3"]; ok {
		assert.EqualValues(t, EnumLongRangeU_Constants__IntOverMax, f3_val)
	} else {
	...
	if f4_val, ok := customMap["F4"]; ok {
		assert.EqualValues(t, EnumLongRangeU_Constants__LongMax, f4_val)
	} else {
...
func TestSerialize_EnumLongRangeU(t *testing.T) {
	...
	original := enum_long_range_u_kaitai.NewEnumLongRangeU()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumLongRangeU(t, original)
	...
	reparsed := enum_long_range_u_kaitai.NewEnumLongRangeU()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_long_range_u_kaitai.EnumLongRangeU{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumLongRangeU converts KSC struct to map for serializer
func structToMapEnumLongRangeU(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumLongRangeU(t, data)
}

// structToMapReflectiveEnumLongRangeU uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumLongRangeU(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumLongRangeU(t, fieldVal.Interface())
