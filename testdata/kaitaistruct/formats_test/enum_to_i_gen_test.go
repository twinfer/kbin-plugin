...
import (
	...
	// The alias helps avoid name collisions if enum_to_i is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumToI(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumToI(t *testing.T, data any) map[string]any {
...
func TestParse_EnumToI(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_to_i.ksy")
	ksySchema := loadKsySchemaForEnumToI(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for enum_to_i, skipping detailed parse tests.")
...
func TestSerialize_EnumToI(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for enum_to_i, skipping detailed serialize tests.")
