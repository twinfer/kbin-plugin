...
import (
	...
	enum_deep_literals_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_deep_literals"
)

func TestParse_EnumDeepLiterals(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_deep_literals.ksy")
	...
	// Assert IsPet1Ok
	if ispet1ok_val, ok := customMap["IsPet1Ok"]; ok {
		assert.EqualValues(t, true, ispet1ok_val)
	} else {
		t.Fatalf("Field 'IsPet1Ok' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert IsPet2Ok
	if ispet2ok_val, ok := customMap["IsPet2Ok"]; ok {
		assert.EqualValues(t, true, ispet2ok_val)
	} else {
		t.Fatalf("Field 'IsPet2Ok' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
...
func TestSerialize_EnumDeepLiterals(t *testing.T) {
	...
	original := enum_deep_literals_kaitai.NewEnumDeepLiterals()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumDeepLiterals(t, original)
	...
	reparsed := enum_deep_literals_kaitai.NewEnumDeepLiterals()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_deep_literals_kaitai.EnumDeepLiterals{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumDeepLiterals converts KSC struct to map for serializer
func structToMapEnumDeepLiterals(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumDeepLiterals(t, data)
}

// structToMapReflectiveEnumDeepLiterals uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumDeepLiterals(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumDeepLiterals(t, fieldVal.Interface())
