...
import (
	...
	// The alias helps avoid name collisions if repeat_n_struct is a common word.
	repeat_n_struct_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/repeat_n_struct"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForRepeatNStruct(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForRepeatNStruct(t *testing.T, data any) map[string]any {
...
func TestParse_RepeatNStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_n_struct.ksy")
	ksySchema := loadKsySchemaForRepeatNStruct(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("repeat_n_struct_repeat_n_struct_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_n_struct.bin")
		...
		kscParsed := repeat_n_struct_kaitai.NewRepeatNStruct() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for repeat_n_struct (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_RepeatNStruct(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_n_struct.ksy")
	ksySchema := loadKsySchemaForRepeatNStruct(t, ksyFilePath)
	...
	t.Run("repeat_n_struct_repeat_n_struct_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_n_struct.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := repeat_n_struct_kaitai.NewRepeatNStruct()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForRepeatNStruct(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := repeat_n_struct_kaitai.NewRepeatNStruct()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for repeat_n_struct (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
