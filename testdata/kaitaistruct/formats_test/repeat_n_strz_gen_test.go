...
import (
	...
	// The alias helps avoid name collisions if repeat_n_strz is a common word.
	repeat_n_strz_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/repeat_n_strz"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForRepeatNStrz(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForRepeatNStrz(t *testing.T, data any) map[string]any {
...
func TestParse_RepeatNStrz(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_n_strz.ksy")
	ksySchema := loadKsySchemaForRepeatNStrz(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("repeat_n_strz_repeat_n_strz_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_n_strz.bin")
		...
		kscParsed := repeat_n_strz_kaitai.NewRepeatNStrz() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for repeat_n_strz (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_RepeatNStrz(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "repeat_n_strz.ksy")
	ksySchema := loadKsySchemaForRepeatNStrz(t, ksyFilePath)
	...
	t.Run("repeat_n_strz_repeat_n_strz_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "repeat_n_strz.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := repeat_n_strz_kaitai.NewRepeatNStrz()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForRepeatNStrz(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := repeat_n_strz_kaitai.NewRepeatNStrz()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for repeat_n_strz (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
