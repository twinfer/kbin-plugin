...
import (
	...
	// The alias helps avoid name collisions if switch_integers is a common word.
	switch_integers_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/switch_integers"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForSwitchIntegers(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForSwitchIntegers(t *testing.T, data any) map[string]any {
...
func TestParse_SwitchIntegers(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_integers.ksy")
	ksySchema := loadKsySchemaForSwitchIntegers(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("switch_integers_switch_integers_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "switch_integers.bin")
		...
		kscParsed := switch_integers_kaitai.NewSwitchIntegers() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for switch_integers (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_SwitchIntegers(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_integers.ksy")
	ksySchema := loadKsySchemaForSwitchIntegers(t, ksyFilePath)
	...
	t.Run("switch_integers_switch_integers_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "switch_integers.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := switch_integers_kaitai.NewSwitchIntegers()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForSwitchIntegers(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := switch_integers_kaitai.NewSwitchIntegers()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for switch_integers (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
