...
import (
	...
	// The alias helps avoid name collisions if switch_else_only is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForSwitchElseOnly(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForSwitchElseOnly(t *testing.T, data any) map[string]any {
...
func TestParse_SwitchElseOnly(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "switch_else_only.ksy")
	ksySchema := loadKsySchemaForSwitchElseOnly(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for switch_else_only, skipping detailed parse tests.")
...
func TestSerialize_SwitchElseOnly(t *testing.T) {
	...
	_ = serializer
	t.Logf("No binary test cases for switch_else_only, skipping detailed serialize tests.")
