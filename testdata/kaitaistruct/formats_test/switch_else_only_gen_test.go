...
func TestParse_SwitchElseOnly(t *testing.T) {
	...
	// Assert PrimByte
	if prim_byte_val, ok := customMap["prim_byte"]; ok {
		assert.EqualValues(t, 102, prim_byte_val)
	} else {
		t.Fatalf("Field 'prim_byte' not found in customMap (keys: %v)", maps.Keys(customMap))
	}
