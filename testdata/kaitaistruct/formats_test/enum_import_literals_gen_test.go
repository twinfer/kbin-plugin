...
import (
	...
	enum_import_literals_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_import_literals"
)

func TestParse_EnumImportLiterals(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_import_literals.ksy")
	...
	// Assert Pet1ToI
	if pet1toi_val, ok := customMap["Pet1ToI"]; ok {
		assert.EqualValues(t, 7, pet1toi_val)
	} else {
		t.Fatalf("Field 'Pet1ToI' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert Pet1Eq
	if pet1eq_val, ok := customMap["Pet1Eq"]; ok {
		assert.EqualValues(t, true, pet1eq_val)
	} else {
		t.Fatalf("Field 'Pet1Eq' not found in customMap (keys: %v)", maps.Keys(customMap))
	}

	// Assert Pet2
	if pet2_val, ok := customMap["Pet2"]; ok {
		assert.EqualValues(t, EnumDeep_Container1_Container2_Animal__Hare, pet2_val)
	} else {
...
func TestSerialize_EnumImportLiterals(t *testing.T) {
	...
	original := enum_import_literals_kaitai.NewEnumImportLiterals()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumImportLiterals(t, original)
	...
	reparsed := enum_import_literals_kaitai.NewEnumImportLiterals()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_import_literals_kaitai.EnumImportLiterals{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumImportLiterals converts KSC struct to map for serializer
func structToMapEnumImportLiterals(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumImportLiterals(t, data)
}

// structToMapReflectiveEnumImportLiterals uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumImportLiterals(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumImportLiterals(t, fieldVal.Interface())
