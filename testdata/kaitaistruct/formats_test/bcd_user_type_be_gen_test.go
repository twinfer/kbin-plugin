...
import (
	...
	// The alias helps avoid name collisions if bcd_user_type_be is a common word.
	bcd_user_type_be_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/bcd_user_type_be"
)

func loadKsySchemaForBcdUserTypeBe(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForBcdUserTypeBe(t *testing.T, data any) map[string]any {
...
func TestParse_BcdUserTypeBe(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "bcd_user_type_be.ksy")
	ksySchema := loadKsySchemaForBcdUserTypeBe(t, ksyPath)
	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("bcd_user_type_be_bcd_user_type_be_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "bcd_user_type_be.bin")
		...
		kscParsed := bcd_user_type_be_kaitai.NewBcdUserTypeBe()
		...
		if diff := cmp.Diff(kscMap, filteredCustomMap, numericComparer, cmpopts.EquateEmpty()); diff != "" {
			t.Errorf("Parser output mismatch for bcd_user_type_be (-want ksc_map, +got filtered_custom_map):\n%s", diff)
...
func TestSerialize_BcdUserTypeBe(t *testing.T) {
	...
	t.Run("bcd_user_type_be_bcd_user_type_be_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "bcd_user_type_be.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)

		goldenStream := kaitai.NewStream(bytes.NewReader(binData))
		goldenStruct := bcd_user_type_be_kaitai.NewBcdUserTypeBe()
		...
		inputMap := structToMapForSerializerForBcdUserTypeBe(t, goldenStruct)
		t.Logf("Serializer inputMap for bcd_user_type_be: %#v", inputMap)
		...
		reparsedStruct := bcd_user_type_be_kaitai.NewBcdUserTypeBe()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct, cmpopts.EquateEmpty()); diff != "" {
			t.Errorf("Serializer output mismatch for bcd_user_type_be (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
