...
func TestParse_BcdUserTypeBe(t *testing.T) {
	...
	ksySchema := loadKsySchemaForBcdUserTypeBe(t, ksyFilePath)
	...
	t.Run("bcd_user_type_be_bcd_user_type_be_Parse", func(t *testing.T) {
		...
		kscParsed := bcd_user_type_be_kaitai.NewBcdUserTypeBe() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for bcd_user_type_be (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})
