...
import (
	...
	// The alias helps avoid name collisions if index_sizes is a common word.
	index_sizes_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/index_sizes"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForIndexSizes(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForIndexSizes(t *testing.T, data any) map[string]any {
...
func TestParse_IndexSizes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "index_sizes.ksy")
	ksySchema := loadKsySchemaForIndexSizes(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("index_sizes_index_sizes_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "index_sizes.bin")
		...
		kscParsed := index_sizes_kaitai.NewIndexSizes() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for index_sizes (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_IndexSizes(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "index_sizes.ksy")
	ksySchema := loadKsySchemaForIndexSizes(t, ksyFilePath)
	...
	t.Run("index_sizes_index_sizes_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "index_sizes.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := index_sizes_kaitai.NewIndexSizes()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForIndexSizes(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := index_sizes_kaitai.NewIndexSizes()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for index_sizes (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
