...
import (
	...
	// The alias helps avoid name collisions if process_to_user is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForProcessToUser(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForProcessToUser(t *testing.T, data any) map[string]any {
...
func TestParse_ProcessToUser(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_to_user.ksy")
	ksySchema := loadKsySchemaForProcessToUser(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for process_to_user, skipping detailed parse tests.")

}

func TestSerialize_ProcessToUser(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "process_to_user.ksy")
	ksySchema := loadKsySchemaForProcessToUser(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for process_to_user, skipping detailed serialize tests.")
