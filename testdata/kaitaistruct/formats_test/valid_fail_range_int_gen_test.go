...
import (
	...
	valid_fail_range_int_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_range_int"
)

func TestParse_ValidFailRangeInt(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_range_int.ksy")
...
func TestSerialize_ValidFailRangeInt(t *testing.T) {
	...
	original := valid_fail_range_int_kaitai.NewValidFailRangeInt()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailRangeInt(t, original)
	...
	reparsed := valid_fail_range_int_kaitai.NewValidFailRangeInt()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_range_int_kaitai.ValidFailRangeInt{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailRangeInt converts KSC struct to map for serializer
func structToMapValidFailRangeInt(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailRangeInt(t, data)
}

// structToMapReflectiveValidFailRangeInt uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailRangeInt(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailRangeInt(t, fieldVal.Interface())
