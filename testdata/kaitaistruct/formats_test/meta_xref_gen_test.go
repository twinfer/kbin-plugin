...
import (
	...
	meta_xref_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/meta_xref"
)

func TestParse_MetaXref(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "meta_xref.ksy")
...
func TestSerialize_MetaXref(t *testing.T) {
	...
	original := meta_xref_kaitai.NewMetaXref()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapMetaXref(t, original)
	...
	reparsed := meta_xref_kaitai.NewMetaXref()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(meta_xref_kaitai.MetaXref{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapMetaXref converts KSC struct to map for serializer
func structToMapMetaXref(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveMetaXref(t, data)
}

// structToMapReflectiveMetaXref uses reflection to convert KSC structs including method calls
func structToMapReflectiveMetaXref(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveMetaXref(t, fieldVal.Interface())
