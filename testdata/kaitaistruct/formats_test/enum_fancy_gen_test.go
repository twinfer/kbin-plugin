...
import (
	...
	enum_fancy_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_fancy"
)

func TestParse_EnumFancy(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_fancy.ksy")
	...
	if pet1_val, ok := customMap["Pet1"]; ok {
		assert.EqualValues(t, EnumFancy_Animal__Cat, pet1_val)
	} else {
	...
	if pet2_val, ok := customMap["Pet2"]; ok {
		assert.EqualValues(t, EnumFancy_Animal__Chicken, pet2_val)
	} else {
...
func TestSerialize_EnumFancy(t *testing.T) {
	...
	original := enum_fancy_kaitai.NewEnumFancy()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumFancy(t, original)
	...
	reparsed := enum_fancy_kaitai.NewEnumFancy()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_fancy_kaitai.EnumFancy{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumFancy converts KSC struct to map for serializer
func structToMapEnumFancy(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumFancy(t, data)
}

// structToMapReflectiveEnumFancy uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumFancy(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumFancy(t, fieldVal.Interface())
