...
import (
	...
	// The alias helps avoid name collisions if enum_fancy is a common word.
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumFancy(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumFancy(t *testing.T, data any) map[string]any {
...
func TestParse_EnumFancy(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_fancy.ksy")
	ksySchema := loadKsySchemaForEnumFancy(t, ksyFilePath)
	...
	_ = interpreter
	t.Logf("No binary test cases for enum_fancy, skipping detailed parse tests.")

}

func TestSerialize_EnumFancy(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_fancy.ksy")
	ksySchema := loadKsySchemaForEnumFancy(t, ksyFilePath)
	...
	_ = serializer
	t.Logf("No binary test cases for enum_fancy, skipping detailed serialize tests.")
