...
import (
	...
	valid_fail_contents_inst_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/valid_fail_contents_inst"
)

func TestParse_ValidFailContentsInst(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "valid_fail_contents_inst.ksy")
...
func TestSerialize_ValidFailContentsInst(t *testing.T) {
	...
	original := valid_fail_contents_inst_kaitai.NewValidFailContentsInst()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapValidFailContentsInst(t, original)
	...
	reparsed := valid_fail_contents_inst_kaitai.NewValidFailContentsInst()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(valid_fail_contents_inst_kaitai.ValidFailContentsInst{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapValidFailContentsInst converts KSC struct to map for serializer
func structToMapValidFailContentsInst(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveValidFailContentsInst(t, data)
}

// structToMapReflectiveValidFailContentsInst uses reflection to convert KSC structs including method calls
func structToMapReflectiveValidFailContentsInst(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveValidFailContentsInst(t, fieldVal.Interface())
