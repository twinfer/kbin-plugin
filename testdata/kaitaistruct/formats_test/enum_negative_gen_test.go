...
import (
	...
	// The alias helps avoid name collisions if enum_negative is a common word.
	enum_negative_kaitai "github.com/twinfer/kbin-plugin/testdata/formats_kaitai_go_gen/enum_negative"
)

// loadKsySchema is a helper to load a .ksy file for the interpreter.
// It's defined once per generated test file.
func loadKsySchemaForEnumNegative(t *testing.T, ksyPath string) *kaitaistruct.KaitaiSchema {
...
func structToMapForSerializerForEnumNegative(t *testing.T, data any) map[string]any {
...
func TestParse_EnumNegative(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_negative.ksy")
	ksySchema := loadKsySchemaForEnumNegative(t, ksyFilePath)

	interpreter, err := kaitaistruct.NewKaitaiInterpreter(ksySchema, nil)
	require.NoError(t, err)

	t.Run("enum_negative_enum_negative_Parse", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_negative.bin")
		...
		kscParsed := enum_negative_kaitai.NewEnumNegative() // Use $ here
		...
		if diff := cmp.Diff(kscMap, customMap); diff != "" {
			t.Errorf("Parser output mismatch for enum_negative (-want ksc_map, +got custom_map):\n%s", diff)
		}
	})

}

func TestSerialize_EnumNegative(t *testing.T) {
	ksyFilePath := filepath.Join("../../../test/formats", "enum_negative.ksy")
	ksySchema := loadKsySchemaForEnumNegative(t, ksyFilePath)
	...
	t.Run("enum_negative_enum_negative_Serialize", func(t *testing.T) {
		samplePath := filepath.Join("../../../test/src", "enum_negative.bin")
		binData, err := os.ReadFile(samplePath)
		require.NoError(t, err)
		...
		goldenStruct := enum_negative_kaitai.NewEnumNegative()
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		inputMap := structToMapForSerializerForEnumNegative(t, goldenStruct)
		...
		// Corrected: Use $ to access fields from the root TemplateData context
		reparsedStruct := enum_negative_kaitai.NewEnumNegative()
		...
		if diff := cmp.Diff(goldenStruct, reparsedStruct); diff != "" {
			t.Errorf("Serializer output mismatch for enum_negative (-want golden_ksc_struct, +got reparsed_ksc_struct):\n%s", diff)
		}
	})
