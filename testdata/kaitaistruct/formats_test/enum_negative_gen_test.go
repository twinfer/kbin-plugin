...
func TestParse_EnumNegative(t *testing.T) {
	// Load schema
	ksyPath := filepath.Join("../../../test/formats", "enum_negative.ksy")
	...
	if f1_val, ok := customMap["F1"]; ok {
		assert.EqualValues(t, EnumNegative_Constants__NegativeOne, f1_val)
	} else {
	...
	if f2_val, ok := customMap["F2"]; ok {
		assert.EqualValues(t, EnumNegative_Constants__PositiveOne, f2_val)
	} else {
...
func TestSerialize_EnumNegative(t *testing.T) {
	...
	original := enum_negative_kaitai.NewEnumNegative()
	err = original.Read(stream, original, original)
	require.NoError(t, err)

	// Serialize and verify round-trip
	inputMap := structToMapEnumNegative(t, original)
	...
	reparsed := enum_negative_kaitai.NewEnumNegative()
	...
	cmpOpts := []cmp.Option{
		...
		cmpopts.IgnoreFields(enum_negative_kaitai.EnumNegative{}, "_io", "_parent", "_root"),
		cmpopts.EquateEmpty(),
	}
...
// structToMapEnumNegative converts KSC struct to map for serializer
func structToMapEnumNegative(t *testing.T, data any) map[string]any {
	// Use reflection to handle KSC structs with method-based values
	return structToMapReflectiveEnumNegative(t, data)
}

// structToMapReflectiveEnumNegative uses reflection to convert KSC structs including method calls
func structToMapReflectiveEnumNegative(t *testing.T, data any) map[string]any {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			if !fieldVal.IsNil() {
				...
				subMap := structToMapReflectiveEnumNegative(t, fieldVal.Interface())
